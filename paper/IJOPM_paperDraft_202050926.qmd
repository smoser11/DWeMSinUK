...
cat("  SD:", round(sample_sd, 4), "\n")
cat("  Range:", round(min(prepared_data$composite_risk, na.rm=T), 4), "-",
    round(max(prepared_data$composite_risk, na.rm=T), 4), "\n")
cat("\nPopulation-Adjusted Estimates (Model-Assisted):\n")
cat("  Mean:", round(pop_mean, 4), "\n")
cat("  SD:", round(pop_sd, 4), "\n")
cat("  Bias Correction:", round(pop_mean - sample_mean, 4), "\n")
```
...
# Print summary
cat("\n=== Model-Assisted Binary Indicator Summary ===\n")
cat("Available indicators:", nrow(forest_data), "\n")
cat("Highest prevalence:", max(forest_data$prevalence_pct, na.rm=T), "%\n")
cat("Lowest prevalence:", min(forest_data$prevalence_pct, na.rm=T), "%\n")
```
...
if (file.exists(here("output", "rds_estimates.RData"))) {
  ...
  if (exists("rds_results") && "980000" %in% names(rds_results)) {
    ...
    cat("\n=== RDS Binary Indicator Summary ===\n")
    cat("Number of indicators:", nrow(rds_summary), "\n")
    cat("RDS-SS estimates range:",
        round(min(rds_summary$rds_ss_est) * 100, 1), "% to",
        round(max(rds_summary$rds_ss_est) * 100, 1), "%\n")

  } else {
    cat("RDS results not found in expected format.\n")
  }
} else {
  cat("RDS results file not found.\n")
}
```
...
if (file.exists(nsum_file)) {
  ...
  if (exists("nsum_results")) {
    cat("NSUM results structure:\n")
    print(names(nsum_results))
    ...
    if (nrow(nsum_summary) > 0) {
      ...
      cat("\n=== NSUM Binary Indicator Summary ===\n")
      cat("Number of indicators:", nrow(nsum_summary), "\n")
      cat("Population prevalence range:",
          round(min(nsum_summary$prevalence) * 100, 1), "% to",
          round(max(nsum_summary$prevalence) * 100, 1), "%\n")

    } else {
      cat("No NSUM indicator estimates found in expected format.\n")
    }
  } else {
    cat("NSUM results object not found.\n")
  }
} else {
  cat("NSUM results file not found. Checking alternative files...\n")
