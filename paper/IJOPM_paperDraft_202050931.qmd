...
format:
  ...
  docx:
    ...
    prefer-html: true  # ADD THIS LINE
  html:
    code-fold: true
    toc: true
    toc-depth: 3
...
For this reason, we used the International Labour Organization’s (@ILO11-indicators)[^1] ‘Indicators of Forced Labour’ to identify the potential for severe labour exploitation and as a basis for the quantification of our labour exploitation and abuse risk index.
...
[^1]: CE: No idea if this is the correct reference -- it is my best guess.
...
Here are the Clustered SS-PSE prevalence estimates by nationality subgroup:

| Subgroup \| Document withholding \| Pay-related issues \|
| 
...
1.  Filipino workers show consistently higher prevalence across all exploitation indicators
2.  Excessive hours shows the highest prevalence in all subgroups
3.  Wide confidence intervals reflect the uncertainty inherent in hidden population estimation
4.  Cluster proportions from MCMC: Filipino (37.4%), Latinx (31.7%), Other (30.9%)

The analysis successfully accounts for nationality-based clustering in the network structure and provides valid subgroup-specific prevalence estimates using the Clustered SS-PSE methodology.
...
Standard SS-PSE assumes that the underlying social network of the hidden population is fully connected.
This means:

-   Any member of the population can be reached by any other member through a path in the network
-   The network forms a single, connected component
-   RDS recruitment can theoretically reach any individual in the population from any starting seed

**Why This Assumption Matters**

The connectedness assumption is fundamental to SS-PSE's theoretical foundation because:

1.  Successive Sampling Logic: SS-PSE models RDS as a "without-replacement random walk through the network"
2.  Depletion Detection: The method assumes that individuals with larger network sizes are recruited earlier, and their depletion over sampling waves provides information about population size
3.  Inclusion Probability Modeling: The probability model relies on the ability to potentially sample any individual from any starting point

**The netclust Solution: Clustered SS-PSE**

The @gamb23-estimating approach addresses clustering through e.g.
Clustered SS-PSE Method (implemented in `netclust` @netclust21-ljgamble):
...
**Why This Matters for the Domestic Worker Study**

In your UK domestic worker study, clustering occurs by nationality (Filipino, Latinx, Other Combined):

1.  Strong recruitment homophily: People tend to recruit others from their own nationality group
2.  Different network structures: Each nationality group may have distinct social network patterns
3.  Varying exploitation patterns: Different groups may experience different types/rates of exploitation
...
-   Avoid the bias and overconfidence that would result from ignoring clustering

```{r}
#| label: netclust-analysis
#| fig-cap: "Clustered SS-PSE prevalence estimates for labour exploitation indicators by nationality subgroup"
#| fig-width: 14
#| fig-height: 10
#| echo: false
#| warning: false
...
# ===== DYNAMIC SUBGROUP TABLE CREATION =====
create_subgroup_table <- function() {
  # Get clean data and overall results
  clean_data <- final_results$clean_data
  overall_results <- final_results$summary_df
  ...
  for(i in 1:length(final_results$individual_results)) {
    ...
    indicator_clean <- case_when(
      indicator_name == "document_withholding_rds" ~ "Document
withholding",
      indicator_name == "pay_issues_rds" ~ "Pay-related issues",
      indicator_name == "threats_abuse_rds" ~ "Threats/abuse",
      indicator_name == "excessive_hours_rds" ~ "Excessive hours",
      indicator_name == "access_to_help_rds" ~ "Limited access to help",
      TRUE ~ indicator_name
    )
    ...
    if(!is.null(mcmc_samples)) {
      N_samples <- mcmc_samples[, "N"]
      p1_samples <- mcmc_samples[, "p 1"]  # Filipino
      p2_samples <- mcmc_samples[, "p 2"]  # Latinx
      p3_samples <- mcmc_samples[, "p 3"]  # Other

      # Calculate cluster population sizes
      N1_samples <- N_samples * p1_samples
      N2_samples <- N_samples * p2_samples
      N3_samples <- N_samples * p3_samples

      # Get trait counts for each cluster
      if(indicator_name %in% colnames(clean_data)) {
        filipino_trait <- sum(clean_data[clean_data$cluster == "Filipino",
 indicator_name], na.rm = TRUE)
        latinx_trait <- sum(clean_data[clean_data$cluster == "Latinx",
indicator_name], na.rm = TRUE)
        other_trait <- sum(clean_data[clean_data$cluster ==
"Other_Combined", indicator_name], na.rm = TRUE)

        # Calculate prevalence estimates (as percentages)
        filipino_prev_samples <- (filipino_trait / N1_samples) * 100
        latinx_prev_samples <- (latinx_trait / N2_samples) * 100
        other_prev_samples <- (other_trait / N3_samples) * 100

        # Get summary statistics
        filipino_prev <- median(filipino_prev_samples)
        filipino_ci <- quantile(filipino_prev_samples, c(0.025, 0.975))

        latinx_prev <- median(latinx_prev_samples)
        latinx_ci <- quantile(latinx_prev_samples, c(0.025, 0.975))

        other_prev <- median(other_prev_samples)
        other_ci <- quantile(other_prev_samples, c(0.025, 0.975))

        # Overall results (convert to percentages)
        overall_row <- overall_results[overall_results$indicator ==
indicator_name, ]
        overall_prev <- overall_row$estimate_pct
        overall_ci_lower <- overall_row$ci_lower_pct
        overall_ci_upper <- overall_row$ci_upper_pct

        # Store results for this indicator
        all_subgroup_results[[indicator_clean]] <- data.frame(
          Subgroup = c("Overall sample", "Filipino", "Latinx", "Other
Combined"),
          RDS_Estimate_Pct = c(overall_prev, filipino_prev, latinx_prev,
other_prev),
          CI_Lower = c(overall_ci_lower, filipino_ci[1], latinx_ci[1],
other_ci[1]),
          CI_Upper = c(overall_ci_upper, filipino_ci[2], latinx_ci[2],
other_ci[2]),
          Sample_n = c(85, 62, 18, 5),
          Trait_n = c(overall_row$n_trait, filipino_trait, latinx_trait,
other_trait),
          stringsAsFactors = FALSE
        )
      }
...
# Create subgroup results
subgroup_results <- create_subgroup_table()

# Format for display table
format_subgroup_table <- function(subgroup_results) {
  # Combine all results
  combined_df <- bind_rows(subgroup_results, .id = "Indicator")

  # Format for table display
  table_df <- combined_df %>%
    mutate(
      `RDS Estimate (%)` = sprintf("%.1f", RDS_Estimate_Pct),
      `95% CI` = sprintf("(%.1f–%.1f)", CI_Lower, CI_Upper)
    ) %>%
    select(Indicator, Subgroup, `RDS Estimate (%)`, `95% CI`)
  
  return(table_df)
}

# Create and display formatted table
display_table <- format_subgroup_table(subgroup_results)

# Display table using kable
knitr::kable(display_table,
             caption = "Clustered SS-PSE prevalence estimates by
nationality subgroup and exploitation indicator",
             booktabs = TRUE, linesep="",
             align = c("l", "l", "r", "r")) %>%
  kable_styling(latex_options = c("striped", "scale_down", 
"hold_position")) %>%
  pack_rows("Document withholding", 1, 4) %>%
  pack_rows("Pay-related issues", 5, 8) %>%
  pack_rows("Threats/abuse", 9, 12) %>%
  pack_rows("Excessive hours", 13, 16) %>%
  pack_rows("Limited access to help", 17, 20) %>%
  footnote(general = "Estimates based on Clustered Sequential Sampling
Population Size Estimation methodology using netclust package. Population
size assumption: 980,000 UK domestic workers. Confidence intervals derived
 from MCMC sampling (1,000 samples, 2,000 burn-in). Analysis accounts for
nationality clustering (Filipino=62, Latinx=18, Other Combined=5) and
network structure in RDS data.",
           general_title = "Note: ",
           escape = FALSE)

# ===== VISUALIZATION 1: Subgroup Comparison Plot =====
# Prepare data for visualization
viz_data <- bind_rows(subgroup_results, .id = "Indicator") %>%
  filter(Subgroup != "Overall sample") %>%  # Focus on subgroups
  mutate(
    Indicator = factor(Indicator, levels = c("Document withholding", 
"Pay-related issues", 
                                           "Threats/abuse", "Excessive
hours", "Limited access to help")),
    Subgroup = factor(Subgroup, levels = c("Filipino", "Latinx", "Other
Combined"))
  )

# Create subgroup comparison plot
subgroup_comparison <- ggplot(viz_data, aes(x = Indicator, y = 
RDS_Estimate_Pct, fill = Subgroup)) +
  geom_col(position = "dodge", alpha = 0.8, width = 0.7) +
  geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), 
                position = position_dodge(width = 0.7), width = 0.25, 
linewidth = 0.5) +
  scale_fill_manual(values = c("Filipino" = "#e74c3c", "Latinx" = 
"#3498db", "Other Combined" = "#95a5a6"),
                    name = "Nationality Group") +
  scale_y_continuous(labels = function(x) paste0(x, "%"),
                     expand = expansion(mult = c(0, 0.1))) +
  labs(
    title = "Exploitation Prevalence by Nationality Subgroup",
    subtitle = "Clustered SS-PSE estimates with 95% confidence intervals",
    x = "Exploitation Indicator",
    y = "Prevalence (%)",
    caption = "Based on RDS sample (n=85) with nationality clustering. 
Population assumption: 980,000."
  ) +
...
# ===== VISUALIZATION 2: Forest Plot by Subgroup =====
forest_data <- bind_rows(subgroup_results, .id = "Indicator") %>%
  filter(Subgroup != "Overall sample") %>%
  mutate(
    Indicator_Subgroup = paste(Indicator, "-", Subgroup),
    Indicator_Subgroup = fct_reorder(Indicator_Subgroup, RDS_Estimate_Pct)
  )

forest_plot <- ggplot(forest_data, aes(x = Indicator_Subgroup, y =
RDS_Estimate_Pct, color = Subgroup)) +
  geom_point(size = 3) +
  geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.2,
linewidth = 1) +
  scale_color_manual(values = c("Filipino" = "#e74c3c", "Latinx" =
"#3498db", "Other Combined" = "#95a5a6"),
                     name = "Nationality Group") +
  scale_y_continuous(labels = function(x) paste0(x, "%"), trans = "log10",
                     breaks = c(0.01, 0.1, 1, 10)) +
  coord_flip() +
  labs(
    title = "Forest Plot: Exploitation Prevalence by Indicator and 
Subgroup",
    subtitle = "Clustered SS-PSE estimates with 95% confidence intervals 
(log scale)",
    x = "Indicator - Nationality Group",
    y = "Prevalence (%) with 95% Confidence Intervals",
    caption = "Filipino workers show consistently higher prevalence across
 indicators"
  ) +
...
# ===== VISUALIZATION 3: MCMC Convergence Diagnostics (FIXED) =====
if(length(final_results$individual_results) > 0) {
  sample_result <- final_results$individual_results[[1]]
  if(!is.null(sample_result$netclust_output$sample)) {
    mcmc_samples <<- sample_result$netclust_output$sample[, "N"]

    # Create trace plot with correct variable name
    trace_data <<- data.frame(
      iteration = 1:length(mcmc_samples),
      N_estimate = mcmc_samples  # Changed from pop_size/var1 to  N_estimate
    )

    trace_plot <- ggplot(trace_data, aes(x = iteration, y = N_estimate )) +
      geom_line(alpha = 0.7, color = "#3498db", linewidth = 0.5) +
      labs(
        title = "MCMC Convergence: Population Size Estimates",
...
cat("\nKey Findings:\n")
cat("- Filipino workers show consistently higher prevalence across all 
indicators\n")
cat("- Excessive hours has highest prevalence in all subgroups\n")
cat("- All MCMC chains converged successfully\n")
cat("- Wide confidence intervals reflect uncertainty in hidden population 
estimation\n")
```

```{r}
#| label: netclust-analysis_old
...
# Display table with proper escaping for LaTeX
knitr::kable(netclust_table,
             caption = "Clustered SS-PSE prevalence estimates for labour
exploitation indicators among domestic workers",
             booktabs = TRUE, linesep="",
             escape = FALSE) %>%
  kable_styling(latex_options = c("striped", "scale_down",
"HOLD_position")) %>%
  footnote(general = "Estimates based on Clustered Sequential Sampling Population Size Estimation methodology using netclust package. Population size assumption: 980,000 UK domestic workers. Confidence intervals derived  from MCMC sampling (1,000 samples, 2,000 burn-in). Analysis accounts for nationality clustering (Filipino, Latinx, Other Combined) and network structure in RDS data.",
           general_title = "Note: ",
           escape = FALSE)
...
if(length(final_results$individual_results) > 0) {
  ...
  if(!is.null(sample_result$netclust_output$sample)) {
    ...
    trace_data <<- data.frame(
      iteration = 1:length(mcmc_samples),
      pop_size = mcmc_samples
    )

    trace_plot <- ggplot(trace_data, aes(x = iteration, y = pop_size)) +
      geom_line(alpha = 0.7, color = "#3498db", linewidth = 0.5) +
      labs(
...
if (nrow(ma_comparison) > 0) {
  ...
  ma_table <- ma_formatted %>%
    ...
    tab_options(
      ...
      table.width = pct(85),
      column_labels.font.size = px(8)  # ADD this
    )
