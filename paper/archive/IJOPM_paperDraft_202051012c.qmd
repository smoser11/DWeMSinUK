...
Respondent-Driven Sampling (RDS) is a network-based sampling method designed to study hidden or hard-to-reach populations [@heckathorn_respondent-driven_1997; @heckathorn_comment_2011; @gile_methods_2018]. It combines chain-referral sampling with a statistical framework to produce population estimates that adjust for network structure and differential recruitment probabilities. The method begins with a set of initial participants ("seeds"), who recruit peers from their social networks. Recruits, in turn, recruit others, creating waves of participation. Each participant reports their network degree (the number of eligible contacts they know), which is used to adjust for selection bias—especially the oversampling of high-degree individuals.

RDS has been widely used to sample hidden populations in public health and social science research [@mccreesh_evaluation_2012; @schwitters_prevalence_2012], and network-based referrals have been described as the only viable method to reach many types of labour trafficking victims [@zhang_measuring_2012]. Recent applications include studies of exploitation among low-wage workers in American cities [@bernhardt_broken_2009], labour trafficking in migrant communities [@vincent_estimating_2017], child labour in India [@zhang_victims_2019], and commercial sexual exploitation in Nepal [@jordan_overcoming_2020].

Our approach can best be described as Web-based RDS [@wejnert_web-based_2008]. We designed a web survey using the JISC online survey interface, suitable for respondents to complete via mobile phone. The main survey was conducted over five months between February and July 2023.
...
## Ethical Approval and Use of AI
...
We analysed the survey sample using multiple estimation models to assess robustness and conduct sensitivity analyses (see Appendix B). The dual structure of our survey instrument—capturing both ego-based and alter-based network information—enables two fundamentally different estimation strategies: Respondent-Driven Sampling (RDS) estimators and Network Scale-Up Methods (NSUM).

To the best of the authors' knowledge, the combined use of both methods in the same study is rare in peer-reviewed academic research, though it has appeared in government-funded reports and technical documents from NGOs and research organizations [@ist20-prevalence; @hick25-human]. This paper contributes to the emerging methodological literature on combining RDS and NSUM in survey research for hidden populations. We also developed a novel three-step bootstrap procedure to address uncertainty in NSUM estimates.



### RDS Estimation

We implemented three RDS estimators, each with distinct assumptions: RDS-I [@salg04-sampling], the Volz–Heckathorn (VH) estimator [@volz08-probability], and Gile's Successive Sampling (RDS-SS) estimator [@gile11-improved]. Given our small sample (85 respondents) and non-negligible sampling fraction, RDS-SS serves as our primary estimator for binary outcomes because it accounts for finite population effects. RDS-I, which performs poorly in small samples and is highly sensitive to seed dependence, is retained as a historical benchmark.

For continuous outcomes such as the exploitation risk index, we employ Model-Assisted (MA-RDS) estimators [@gile15-network]. MA-RDS is design-based yet leverages a working Exponential Random Graph Model (ERGM) with degree and homophily terms to approximate inclusion probabilities conditional on observed seeds. This approach directly addresses seed bias and homophily that conventional RDS estimators cannot correct, accommodates finite-population effects through successive-sampling logic, and supports valid estimation of both binary and continuous outcomes. By conditioning on actual seed composition and subgroup structure, the method reduces bias, improves efficiency, and provides design-compatible bootstrap uncertainty [@gile11-improved].

### NSUM Estimation

We employ the Modified Basic Scale-Up (MBSU) model [@feeh16-generalizing], which improves upon the original Basic Scale-Up Method by accounting for key sources of bias. The traditional method assumes that respondents can accurately report how many people they know in the hidden population and that all members are equally likely to be known. These assumptions often fail due to stigma, recall issues, or social barriers.

The MBSU addresses this through two primary adjustments. First, the degree ratio adjustment (δ) accounts for differences in network size between the hidden population and frame population. Second, the visibility/true positive rate adjustment (τ) corrects for underreporting of connections to hidden population members due to stigma or lack of awareness. We apply these adjustments with sensitivity analysis across multiple parameter values: δ ∈ {0.8, 0.9, 1.0} and τ ∈ {0.7, 0.85, 1.0}, where δ=1.0 and τ=1.0 represent the unadjusted baseline scenario.

### Comparative Rationale: Ego-Based vs. Alter-Based Inference
...
### Bootstrap and Sensitivity Procedures

To appropriately characterise uncertainty in NSUM estimates, we developed a novel three-step bootstrap procedure. This approach resamples respondents, their reported alters, and the exploitation classifications simultaneously, thereby capturing uncertainty at each stage of the inference process. This procedure provides more realistic confidence intervals than those generated by conventional variance estimators, particularly for small samples such as ours. Details of the bootstrap implementation and diagnostic checks are provided in Appendix C.

For RDS estimators, we employ neighborhood bootstrap methods that preserve the network structure while resampling [@yauc22-neighborhood]. Sensitivity analyses examine robustness to population size assumptions (50K, 100K, 980K, 1.74M) and NSUM adjustment factor values. Full results are reported in Appendix B.
...
Sample characteristics are presented in @tbl-sample-characteristics and @fig-rds-network (Section 3.3). In total, 97 respondents completed the survey, of whom 85 had non-zero network degree and were included in RDS analysis. The analytical sample comprised 90 transnational migrants (93%), with the largest nationality cluster being Filipino (66%). Female domestic workers constituted 97% of the sample, with over half aged 45 or older. Self-employment status was reported by 45% of respondents.

## Model-Assisted Estimates of the Continuous Risk Index
...
Because continuous traits cannot be estimated directly with conventional RDS estimators or NSUM, we employed model-assisted inference methods [@gile15-network]. These methods adjust for the non-random structure of RDS recruitment while permitting reliable estimation of means and distributions of continuous outcomes.
...
## Binary Exploitation Indicators: Comparative RDS–NSUM Estimates

To complement the continuous measure, we operationalised exploitation as binary outcomes. Respondents were classified as exploited if they met threshold indicators consistent with ILO definitions. We present prevalence estimates from three approaches: (1) Model-Assisted (MA-RDS) Bayesian estimates that adjust for RDS sampling bias, (2) frequentist RDS estimators (RDS-I and RDS-SS) for comparison, and (3) Network Scale-Up Method (NSUM) estimates with multiple adjustment factor scenarios.

```{r}
#| label: fig-method-comparison
#| fig-cap: "Comparative Method Overview: Point estimates and uncertainty intervals for prevalence of exploitation indicators across different estimation methods. RDS methods show 95% confidence intervals (bootstrap); Bayesian methods show 95% credible intervals (MCMC); NSUM shows estimates under different adjustment scenarios. Baseline population assumption: 980,000 domestic workers. Source: Authors' own work."
#| echo: false
#| out-width: "95%"
#| fig-align: "center"
#| dpi: 300

# Format-specific figure inclusion with higher resolution
...
@fig-method-comparison provides a comprehensive comparison of prevalence estimates across all three approaches. While absolute values differ—reflecting the fundamental differences between ego-based (RDS) and alter-based (NSUM) inference—the overall pattern is remarkably stable. Excessive hours consistently shows the highest prevalence across all methods, followed by threats/abuse and pay-related issues. Model-Assisted estimates generally fall between RDS frequentist estimates and NSUM baseline estimates, though NSUM estimates are highly sensitive to adjustment factor assumptions.

### Model-Assisted Bayesian Estimates

```{r}
#| label: tbl-ma-binary-indicators
#| tbl-cap: "Model-Assisted Bayesian Estimates of Binary Exploitation Indicators: Population prevalence estimates adjusted for RDS sampling bias using Bayesian inference. Values represent estimated proportions with 95% credible intervals. Source: Authors' Own Work."
#| echo: false
#| warning: false
#| message: false
...
ma_results_formatted %>%
  ...
  kableExtra::footnote(
    general = "Estimates adjusted for RDS sampling bias using Bayesian Model Assisted Inference (Gile & Handcock, 2015). N=980,000; sample seed selection. Credible intervals from posterior distribution.",
    general_title = "Note:",
    footnote_as_chunk = TRUE,
    threeparttable = TRUE
  )
...
### Frequentist RDS Estimates

@tbl-rds-binary-indicators compares RDS-I and RDS-SS estimators for binary exploitation indicators. As expected, RDS-SS produces slightly more conservative estimates than RDS-I due to its finite population correction. Both estimators show similar relative patterns across indicators, with @fig-rds-forest-plot visualizing the comparison.

```{r}
#| label: tbl-rds-binary-indicators
#| tbl-cap: "Frequentist RDS Estimates of Binary Exploitation Indicators: Comparison of RDS-I and RDS-SS estimators with neighborhood bootstrap confidence intervals (N=980,000). Source: Authors' Own Work."
#| echo: false
#| warning: false
#| message: false
...
if (file.exists(here("output", "frequentist_estimators_results.RData"))) {
  ...
  if (exists("frequentist_df")) {
    ...
    rds_comparison %>%
      ...
      kableExtra::footnote(
        general = "RDS-I: Salganik-Heckathorn estimator. RDS-SS: Gile's Successive Sampling estimator. Confidence intervals from neighborhood bootstrap (n=1000). Population: 980,000 domestic workers.",
        general_title = "Note:",
        footnote_as_chunk = TRUE,
        threeparttable = TRUE
      )
...
### Network Scale-Up Method (NSUM) Estimates

Using the Modified Basic Scale-Up (MBSU) estimator [@feeh16-generalizing] with Sequential Sampling weights and a UK domestic worker population of 980,000, @tbl-nsum-main presents prevalence estimates under three adjustment scenarios: (1) unadjusted baseline (δ=1.0, τ=1.0), (2) moderate adjustments (δ=0.9, τ=0.85), and (3) conservative adjustments (δ=0.8, τ=0.7).

NSUM estimates show substantial sensitivity to adjustment factor assumptions. Unadjusted estimates range from 0.83% (document withholding) to 2.28% (excessive hours). Conservative adjustments accounting for network degree differences and visibility barriers produce estimates approximately 1.8 times higher—ranging from 1.48% to 4.08%—while moderate adjustments yield intermediate values approximately 1.3 times the baseline. Wide confidence intervals reflect substantial uncertainty inherent in network-based estimation methods, particularly for small hidden populations.
...
if (file.exists(nsum_file)) {
  ...
  # Select only the 6 essential columns to display
  nsum_main_clean <- nsum_main %>%
    dplyr::select(1:6)  # Select first 6 columns only

  # Format the table - NO caption parameter (using tbl-cap from chunk options)
  table_output <- nsum_main_clean %>%
    kbl(
    ...
    footnote(
      general = "MBSU = Modified Basic Scale-Up method; δ = degree ratio (d̄_H,F/d̄_F,F); τ = visibility/true positive rate (v̄_H,F/d̄_H,F); Bootstrap: neighborhood resampling (n=1000); Population: 980,000 UK domestic workers.",
      general_title = "Note:",
      footnote_as_chunk = TRUE,
      threeparttable = TRUE
    )
...
### Interpreting RDS–NSUM Divergence: The Role of Visibility
  ...
  if (exists("frequentist_df")) {
    ...
    table_out <- rds_comprehensive %>%
      ...
      kableExtra::kable_styling(
        ...
        font_size = if (knitr::is_latex_output()) 7 else NULL,
        full_width = FALSE
      ) %>%
    ...
    # Add landscape wrapper for PDF using kableExtra function
    if (knitr::is_latex_output()) {
      table_out %>% kableExtra::landscape()
    } else {
      table_out
    }
  } else {
