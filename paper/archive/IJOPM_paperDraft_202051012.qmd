---
title: "Quantifying Hidden Exploitation: Dual-Method Prevalence Estimates of Modern Slavery Risk Among UK Domestic Workers^[Authors' names are listed in alphabetical order.  Replication materials are available from the authors upon request. _Acknowledgements_: **AI DISCLAIMER HERE**]"
author:
...
<!-- [^11]: <a name="footnote1"></a>our detailed footnote text goes here. -->
...
Along with this novel conceptualization of labour exploitation (LE) as a continuous _risk index_ (or risk score -- see Appendix A for details of its construction) we also employ a series of dichotomous indicators.
TRIANGLE
...
**CAROLINE: DO WE NEED THE FOLLOWING PARAGRAPH?**
...
Overseas domestic worker visa holders are now, again, permitted to change employers, but not to apply to renew their six-month long visa unless they receive a positive ‘Conclusive Grounds’ decision related to exploitation considered to be modern slavery through the UK’s NRM [@romero_blueprint_2025].
...
Comprehensive descriptions and literature reviews of the development and use of RDS to estimate the population size of a hidden population are available elsewhere (@heckathorn_comment_2011; @gile_methods_2018).  Here is the idea:  Respondent-Driven Sampling (RDS) is a network-based sampling method designed to study hidden or hard-to-reach populations, such as sex workers, drug users, or undocumented workers. It combines snowball sampling with a statistical framework to produce population estimates that adjust for network structure and differential recruitment probabilities.
...
RDS was introduced by Heckathorn as an extension of traditional chain-referral sampling, with theoretical justification and estimation procedures developed in subsequent work [@heckathorn_respondent-driven_1997; @gile11-improved].

Following the identification of a set of original sample members known as seeds, ; @heckathorn_respondent-driven_2002 advocate the use of a double incentive to recompense participants not only for their involvement, but also for their recruitment of further participants in subsequent ‘waves’ of participation by drawing upon the social ties through which members of the hidden population are connected to each other.  Though they may be subject to both systematic and non-systematic errors, the use of snowballing methods for the study of hidden populations, with the support of monetary or symbolic rewards, has been advocated as a way of creating robust recruitment embodying diversity in characteristics such as ethnicity, gender and geographical location (@heckathorn_respondent-driven_1997; @heckathorn_respondent-driven_2002).
...
RDS also takes a non-random approach to seed selection, but relies upon the social network structure that exists between participants to produce a non-probabilistic sample [@goodman_comment_2011].   Adjusting for the lack of 'at random' sampling is accomplished in a number or ways, boradly by employing _weights_ and/or _modeling_ the recruitment mechanism [@gile15-network]

RDS has been widely used to sample a variety of hidden populations, including HIV prevalence, rape and client-initiated gender-based violence among sex workers [@mccreesh_evaluation_2012; @schwitters_prevalence_2012].  Such network-based referrals have been described as the only viable method to reach many types of labour trafficking victims [@zhang_measuring_2012] and have been used to research exploitation among low-wage workers in three American cities [@bernhardt_broken_2009]; a study of labour trafficking in migrant communities in the city of San Diego [@vincent_estimating_2017]; examination of the worst forms of child labour in the Indian state of Bihar [@zhang_victims_2019]; and the commercial sexual exploitation of children in Nepal [@jordan_overcoming_2020].
...
The survey consisted of these 11 composite indicators and included questions related to domestic workers’ level of job satisfaction, employment conditions, and demographic data such as nationality, age, and gender.  Based on these, we employ five dichotomous indicators and one continuous indicator of LE.  See Appendix A for details of these survey questions, and the construction of comparable ego- and alter-based questions. The main survey was conducted in the five months between February and July 2023.
...
While respondents were asked to nominate up to 10 domestic workers within their existing social network, it was the up to five of these from whom participation was requested in subsequent waves.
...
Forty-five percent regarded themselves as self-employed, 39% identified themselves as employees, and 16% categorised their employment status as that of a worker.  Of the 97 respondents, 64 (66% of the total), and the largest single nationality group, reported that they had a Filipina background.
...
The age structure of the domestic workers was skewed towards those over 45 years old, with such workers representing over half of the sample (see @tbl-sample-characteristics).

@fig-rds-network presents the recruitment network in our sample.  Twelve partiicpants reported know zero domestic workers in the UK -- that is, having a null social network -- had to be removed.
...
# Format table (NO caption - using tbl-cap from chunk options)
sample_summary %>%
  kableExtra::kbl(
...
To the best of the authors' knowledge, the use of both methods in the same study is limited in academic research (though we note the dual use in Government-funded research reports and Technical reports from NGOs and research organizations -- e.g. @ist20-prevalence and @hick25-human).  Hence this paper contributes to the emergine area combinating RDS and NSUM  in survey methodology for hidden populations. The relative scarcity of peer-reviewed publications may reflect: The methodological complexity of implementing both approaches;
The specialized nature of applications (primarily human trafficking and other sensitive populations).

While there is significant interest in combining RDS and NSUM methodologies, particularly for studying hidden and marginalized populations, the number of peer-reviewed academic publications specifically documenting their simultaneous use in the same survey appears to be quite limited.

## Estimation Framework

We analysed the survey sample using multiple estimation models in order to assess robustness and conduct sensitivity analyses (see Appendix B).
...
We implemented RDS-I and Gile’s successive sampling (SS) estimator, the latter of which accounts for finite population effects and improves performance when the sample fraction is relatively large (Gile, 2011; Gile and Handcock, 2010).
...
First, the RDS-I estimator @salg04-sampling provides an early design-based approach.
...
Since our sample comprises fewer than 100 respondents recruited from multiple seeds, we regard RDS-I primarily as a historical benchmark rather than a viable option for inference.[^ The Volz–Heckathorn (VH) estimator @volz08-probability offers a probability-based refinement of RDS-I.
...
<<<<<<< HEAD
Next, the Successive Sampling (RDS-SS) estimator [@gile11-improved] accounts for finite population effects, adjusting for the non-negligible sampling fraction that arises when the target population is not extremely large relative to the sample.
=======
Next, the Successive Sampling (RDS-SS) estimator (also called RDS-II) @gile11-improved accounts for finite population effects, adjusting for the non-negligible sampling fraction that arises when the target population is not extremely large relative to the sample.
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
...
To estimate continuous outcomes such as our exploitation risk index, we employ the Model-Assisted (MA-RDS) estimator @gile15-network. MA inference uses realistic network simulations to better estimate how likely each kind of person was to be sampled, improving population estimates over standard RDS estimators.



In summary, we rely primarily on RDS-SS for binary outcomes and MA-RDS for continuous and binary indicators, with RDS-I used for historical benchmarks.  This combined strategy balances methodological rigor with the substantive goals of our study.
...
The Modified Basic Scale-Up (MBSU) model, introduced by Feehan and Salganik (2016), improves upon the original Basic Scale-Up Method (BSU) for estimating the size of hidden populations by accounting for key sources of bias. The traditional BSU method assumes that members of the general population can accurately report how many people they know in the hidden population and that all members of the hidden group are equally likely to be known. These assumptions often fail in practice due to stigma, recall issues, or social barriers that affect how people perceive and report their connections. The MBSU addresses this by introducing adjustment factors that correct for differential visibility and reporting behavior.

Two primary adjustments are central to the MBSU model. First, the visibility adjustment accounts for the fact that not all members of the hidden population are equally likely to be known by others; it is estimated using data from the hidden population itself, who report how many people are aware of their membership in the group. Second, the reporting adjustment corrects for the tendency of general population respondents to underreport their known connections to hidden population members, often due to stigma or social desirability bias. This adjustment is typically estimated by comparing reporting accuracy across groups with known sizes. By incorporating these two corrections, the MBSU provides more accurate estimates of hidden population sizes under realistic social conditions, especially when the assumptions of the basic scale-up model are violated.
...
Details of the bootstrap implementation and diagnostic checks are provided in Appendix C.
...
RDS uses ego-based information.  That is, information about survey respondent's attributes and characteristics. Each participant reports the size of their personal network of eligible individuals, and these degree reports are combined with the wave at which respondents were recruited to adjust for unequal inclusion probabilities. The underlying logic is that individuals with larger networks are more likely to be recruited earlier and more often, creating a bias toward highly connected respondents. However, is is not a simple random sample and so we must account for non-random selection into the survey.  RDS estimators, including Gile’s successive sampling estimator, explicitly correct for this bias by weighting observations according to network degree and recruitment path. For continuous traits, such as our Risk Index, RDS model-assisted estimators incorporate auxiliary covariates into this weighting process to further reduce bias.

In contrast, NSUM uses alter-based information. Rather than focusing on the ego’s probability of inclusion, NSUM treats respondents as informants about the wider hidden population. Respondents are asked how many people they know with a given trait (for example, “Do you know personally of any other domesticc workers that are currently experiencing any restrictions on their freedom? If so, how many?”). These responses are then scaled up, using assumptions about network size and visibility, to infer prevalence in the broader population. This method does not depend on recruitment chains but instead on the accuracy of respondents’ knowledge about others in their social circles.
...
Rather than treating exploitation as a dichotomy, the risk index conceptualises all domestic workers as facing some degree of potential exploitation, albeit with significant variation in intensity.  Conceptually, the presumes that everyone in the sampling frame are exposed to some degree of LE or risk of exploitation.  Emperically this is all domestic workers in the UK with a  mobile phone (during the survey dates).
...
# Create professional table (NO caption - using tbl-cap from chunk options)
...
The most conservative visability and reporting adjustments (δ $\delta=0.8$, τ$\tau=0.7$), accounting for greater underreporting and visibility barriers, produce substantially higher estimates: 14,500 workers with document withholding (1.48%, 95% CI: 0.40%-4.27%), 38,499 with pay issues (3.93%, 95% CI: 2.60%-7.77%), 35,966 experiencing threats/abuse (3.67%, 95% CI: 2.53%-8.00%), 39,955 with excessive hours (4.08%, 95% CI: 2.52%-7.53%), and 27,165 lacking access to help (2.77%, 95% CI: 0.91%-6.67%).
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
...
#| label: tbl-nsum-main
<<<<<<< HEAD
#| tbl-cap: "NSUM Prevalence Estimates for Labor Exploitation Indicators: Estimates using Modified Basic Scale-Up (MBSU) method with Sequential Sampling weights and population size of 980,000 domestic workers. Adjustment factors: $\\delta$ (δ, degree ratio) controls for network size differences; $\\tau$ (τ, visibility/true positive rate) controls for reporting accuracy and visibility barriers. Bootstrap confidence intervals based on neighborhood resampling (B=1000). Source: Authors' Own Work."
=======
#| tbl-cap: "NSUM Prevalence Estimates for Labor Exploitation Indicators: Estimates using Modified Basic Scale-Up (MBSU) method with Sequential Sampling weights and population size of 980,000 domestic workers. Adjustment factors: δ $\\delta$ (transmission/barrier effect) and τ $\\tau$(recall effect) control for visibility and reporting biases. Bootstrap confidence intervals based on neighborhood resampling (B=1000). Source: Authors' Own Work."
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
#| echo: false
#| warning: false
#| message: false
...
if (file.exists(nsum_file)) {
  ...
  # Format the table - NO caption parameter (using tbl-cap from chunk options)
  table_output <- nsum_main %>%
    kbl(
    ...
    kable_styling(
      latex_options = if (knitr::is_latex_output()) {
        c("striped", "hold_position", "scale_down", "repeat_header")
      } else {
        NULL
      },
    ...
    footnote(
<<<<<<< HEAD
      general = "MBSU = Modified Basic Scale-Up method; δ = degree ratio (d̄_H,F/d̄_F,F, ratio of hidden population network degree to frame population degree); τ = visibility/true positive rate (v̄_H,F/d̄_H,F, proportion of edges correctly reported, measures transmission and reporting accuracy); Bootstrap method: neighborhood resampling with 1000 iterations; Population base: 980,000 UK domestic workers.",
=======
...
<<<<<<< HEAD
While absolute values differ across RDS and NSUM (reflecting differences in ego- versus alter-based assumptions), the overall pattern is remarkably stable.

@fig-method-comparison demonstrates the comparison between frequentist RDS estimators (RDS-I, RDS-SS) and Bayesian model-assisted methods across all exploitation indicators.
=======
...
# Format-specific figure inclusion
fig_path <- here::here("output", "figures", "method_comparison.png")

if (file.exists(fig_path)) {
  if (knitr::is_html_output()) {
    knitr::include_graphics(fig_path)
  } else {
    # For PDF output with specific dimensions
    knitr::include_graphics(fig_path)
  }
} else {
  cat("Figure file not found:", fig_path)
}
```
...
<!-- > “Transmission error occurs when respondents do not know or are unwilling to report the true group memberships of people they know” [@feeh16-generalizing, p. 155]. -->

This issue has been recognized since the earliest implementations of NSUM. Bernard et al. describe it as a failure of the transmission condition—the assumption that a respondent will be aware of, and willing to disclose, the group membership of their contacts [@bern10-counting]. More recently, Maltiel et al. have addressed visibility bias using model-based (Bayesian) approaches, which estimate and correct for differential visibility between groups [@malt15-estimating].

In contrast, RDS estimators such as the Volz–Heckathorn and especially the Successive Sampling (SS) estimator rely on direct self-reporting of hidden group membership. That is, RDS respondents are asked whether they themselves belong to the group or exhibit a trait of interest. This egocentric measurement bypasses the visibility problem entirely. While RDS relies on its own set of assumptions—such as the random recruitment assumption, accurate self-reported degree, and a sufficiently connected social network—visibility is not among its main sources of bias.

Thus, while both NSUM and RDS are used to estimate the size or prevalence of hidden populations, they are subject to fundamentally different sources of error. In NSUM, low visibility leads to systematic underreporting, which can result in severe underestimation unless appropriately adjusted. Adjustments such as those proposed by Feehan and Salganik—including multiplicative correction factors for degree ratio ($\delta$ δ, network size differences), visibility/true positive rate ($\tau$ τ, transmission and reporting accuracy), and frame ratio ($\phi_F$ φ_F)—can mitigate these biases, but require additional data or assumptions [@feeh16-generalizing].

In our setting, where RDS estimates suggest that over 50% of respondents have experienced some form of labour exploitation, but NSUM estimates suggest only 3–5% of the domestic worker population does so, visibility bias is almost certainly the dominant factor explaining the discrepancy. This is especially true in contexts involving exploitation, abuse, or trafficking—where group membership is often concealed both by victims and by observers. Respondents in the frame population may simply be unaware that their contacts are experiencing exploitation, leading to substantial underreporting.
=======
On the Role of Visibility in Explaining Differences Between NSUM and RDS Estimates
----------------------------------------------------------------------------------
...
<<<<<<< HEAD
A series of robustness checks were performed to assess the stability of the findings.  In Appendix B,  a comprehensive analysis comparing all prevalence estimates across indicators, methods, and parameter values is presented.  There,  remarkable consistency is demonstrated across different analytical approaches.
=======
...
<<<<<<< HEAD
_Dual conceptualisation of exploitation_: We introduce two distinct approaches to operationalising exploitation.
=======
...
<<<<<<< HEAD
Network scale up methods, and potential enhancements such as Generalised network scale up estimators offer to enhance understanding, not least within operations and supply chain management research, of the extent of labour exploitation in different sectors and across industries.  The differences in NSUM and RDS estimates warrants further investitation and suggests research into _combining_ multiple estimates may be a fruitful direction in human rights research and especially prevelenace of modern slavery research.
...
The core issue is that the two methods rely on differently framed questions:
...
The goal is to aggregate some of the specific "egocentric" questions to make them comparable to the broader "how many do you know" NSUM questions.  To accomplish this, several sets of "linking" questions have been identified with varying degrees of confidence in their comparability:
...
Based on our sources, several sets of "linking" questions have been identified with varying degrees of confidence in their comparability: • Document Withholding (High Confidence): ◦ RDS Question: Q70 asks if the employer has withheld the respondent's travel and identity documents.
◦ NSUM Question: Q71 asks how many other domestic workers the respondent knows who do not have access to their own documents.
• Debt and Pay Issues (High Confidence): ◦ RDS Questions: This involves combining Q39 (having to pay off debt to someone who helped find work) and Q42 (has our pay ever been withheld?).
◦ NSUM Question: Q43 asks how many others the respondent knows who have experienced problems with debt or pay.
...
if (file.exists(here("output", "frequentist_estimators_results.RData"))) {
  ...
  if (exists("frequentist_df")) {
    ...
    # Format with landscape for PDF, regular for HTML
    table_out <- rds_comprehensive %>%
      kableExtra::kbl(
        col.names = col_names,
        booktabs = TRUE,
...
<<<<<<< HEAD
This section presents comprehensive sensitivity analysis for Model-Assisted inference estimates across all exploitation indicators, examining robustness to population size assumptions and seed selection methods.  @fig-ma-sensitivity-by-indicator and @fig-ma-sensitivity-comparison show MA estimates are robust to these paramter choices.
=======
...
<<<<<<< HEAD
The choice of NSUM estimator and adjustment factors had substantial impact on estimates (@tbl-appendix-nsum-method). We compared MBSU with three adjustment factor sets (no adjustment, moderate, conservative) and GNSUM Symmetric estimation. GNSUM estimates matched MBSU (No Adjustment) exactly for all indicators, while conservative MBSU adjustments (δ$\delta=0.8$ τ$\tau=0.7$) produced estimates approximately 1.8 times higher than unadjusted estimates. Moderate adjustments (δ=0.9, τ=0.85) provided intermediate values approximately 1.3 times higher than baseline, representing our preferred specification that acknowledges likely visibility and reporting barriers while avoiding extreme assumptions.
=======
...
# Appendix C: Bootstrap Estimation for Network Scale-Up Using RDS Data {#sec:3step}
...
As shown in @feeh16-generalizing and @salg06-variance, the NSUM estimator depends crucially on inclusion weights $\pi_i$, which must reflect the sampling design.
<<<<<<< HEAD
When data are RDS-based, these weights are typically derived from known degree-based estimators such as RDS-I, or RDS-II or Gile’s Successive Sampling (SS) weights.
=======
...
-   Successive Sampling Bootstrap (SSB): Sample with replacement according to inclusion probabilities derived from the SS model [@gile11-improved].
-   Neighborhood Bootstrap: Use ego-network topology to preserve recruitment ties and neighborhood structure [@yauc22-neighborhood].
...
network is finite and connected ii) network connec5ons are reciprocal, not directed iii) recruits accurately report their network degree [this is
...
random to neighbours in the network   v) that
...
-   RDS-II Weights [@volz08-probability]: $$
    w_i^{(b)} \propto \frac{1}{d_i^{(b)}}
    $$

-   SS Weights [@gile11-improved]: Incorporate sampling fraction and frame size $N_F$.
...
As described in @feeh16-generalizing, GNSUM estimates:
...
In our context, some RDS respondents are ex post identified as members of the hidden population.
...
<<<<<<< HEAD
-   $\delta$ (δ): Degree ratio (d̄_H,F/d̄_F,F) — ratio of average network degree of hidden population to frame population,

-   $\tau$ (τ): Visibility / true positive rate (v̄_H,F/d̄_H,F) — proportion of network edges correctly reported; measures transmission and reporting accuracy; captures visibility of hidden population to frame population,

-   $\rho$ (ρ): Popularity bias (relative visibility of hidden population members).
=======
