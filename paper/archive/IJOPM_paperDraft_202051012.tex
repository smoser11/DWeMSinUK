...
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
...
of Modern Slavery Risk Among UK Domestic Workers\footnote{Authors' names
  are listed in alphabetical order. Replication materials are available
  from the authors upon request. \emph{Acknowledgements}: \textbf{AI
  DISCLAIMER HERE}}}
...
working conditions may be considered exploitative.

Along with this novel conceptualization of labour exploitation (LE) as a
continuous \emph{risk index} (or risk score -- see Appendix A for
details of its construction) we also employ a series of dichotomous
indicators. TRIANGLE
...
\textcite{gile_methods_2018}). Here is the idea: Respondent-Driven
Sampling (RDS) is a network-based sampling method designed to study
hidden or hard-to-reach populations, such as sex workers, drug users, or
undocumented workers. It combines snowball sampling with a statistical
framework to produce population estimates that adjust for network
structure and differential recruitment probabilities.

The method begins with a set of initial participants (``seeds''), who
...
RDS was introduced by Heckathorn as an extension of traditional
chain-referral sampling, with theoretical justification and estimation
procedures developed in subsequent work
\autocite{heckathorn_respondent-driven_1997,gile11-improved}.

Following the identification of a set of original sample members known
as seeds, ; \textcite{heckathorn_respondent-driven_2002} advocate the
use of a double incentive to recompense participants not only for their
...
other. Though they may be subject to both systematic and non-systematic
errors, the use of snowballing methods for the study of hidden
populations, with the support of monetary or symbolic rewards, has been
advocated as a way of creating robust recruitment embodying diversity in
characteristics such as ethnicity, gender and geographical location
...
\autocite{gile_network_2015}. Snowball sampling is based upon the
initial recruitment of the original sample selection by means of
...
\autocite{goodman_comment_2011}. Adjusting for the lack of `at random'
sampling is accomplished in a number or ways, boradly by employing
\emph{weights} and/or \emph{modeling} the recruitment mechanism
\autocite{gile15-network}
...
\autocite{mccreesh_evaluation_2012,schwitters_prevalence_2012}. Such
network-based referrals have been described as the only viable method to
reach many types of labour trafficking victims
\autocite{zhang_measuring_2012} and have been used to research
exploitation among low-wage workers in three American cities
\autocite{bernhardt_broken_2009}; a study of labour trafficking in
migrant communities in the city of San Diego
...
nationality, age, and gender. Based on these, we employ five dichotomous
indicators and one continuous indicator of LE. See Appendix A for
details of these survey questions, and the construction of comparable
ego- and alter-based questions. The main survey was conducted in the
five months between February and July 2023.
...
that of a worker. Of the 97 respondents, 64 (66\% of the total), and the
largest single nationality group, reported that they had a Filipina
background. Other nationalities represented included Dominican,
...
Figure~\ref{fig-rds-network} presents the recruitment network in our
sample. Twelve partiicpants reported know zero domestic workers in the
UK -- that is, having a null social network -- had to be removed.

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-rds-network-1.pdf}
...
\section{Estimation Strategies}\label{estimation-strategies}

We take two different strategies for estimation: RDS and NSUM.

To the best of the authors' knowledge, the use of both methods in the
same study is limited in academic research (though we note the dual use
in Government-funded research reports and Technical reports from NGOs
and research organizations -- e.g. \textcite{ist20-prevalence} and
\textcite{hick25-human}). Hence this paper contributes to the emergine
area combinating RDS and NSUM in survey methodology for hidden
populations. The relative scarcity of peer-reviewed publications may
reflect: The methodological complexity of implementing both approaches;
The specialized nature of applications (primarily human trafficking and
other sensitive populations).

While there is significant interest in combining RDS and NSUM
methodologies, particularly for studying hidden and marginalized
populations, the number of peer-reviewed academic publications
specifically documenting their simultaneous use in the same survey
...
to assess robustness and conduct sensitivity analyses (see Appendix B).
...
individuals in the sample. We implemented RDS-I and Gile's successive
...
benchmark rather than a viable option for inference.\autocite[\^{} The
Volz--Heckathorn (VH) estimator][ offers a probability-based refinement
of RDS-I. It is more robust but assumes a large population relative to
the sample size and does not adjust for finite-population effects.
Although our diagnostic checks indicated approximate equilibrium across
key demographics, the VH estimator is best used here as a robustness
check rather than the primary estimator.]{volz08-probability} Next, the
<<<<<<< HEAD
Successive Sampling (RDS-SS) estimator \autocite{gile11-improved}
accounts for finite population effects, adjusting for the non-negligible
sampling fraction that arises when the target population is not
extremely large relative to the sample. This property makes RDS-SS
particularly appropriate for our study of migrant domestic workers, and
we therefore use it as our primary estimator for binary outcomes (e.g.,
presence or absence of exploitation).
=======
Successive Sampling (RDS-SS) estimator (also called RDS-II)
...
To estimate continuous outcomes such as our exploitation risk index, we
employ the Model-Assisted (MA-RDS) estimator \textcite{gile15-network}.
MA inference uses realistic network simulations to better estimate how
likely each kind of person was to be sampled, improving population
estimates over standard RDS estimators.

In summary, we rely primarily on RDS-SS for binary outcomes and MA-RDS
for continuous and binary indicators, with RDS-I used for historical
benchmarks. This combined strategy balances methodological rigor with
the substantive goals of our study.
...
The Modified Basic Scale-Up (MBSU) model, introduced by Feehan and
Salganik (2016), improves upon the original Basic Scale-Up Method (BSU)
for estimating the size of hidden populations by accounting for key
sources of bias. The traditional BSU method assumes that members of the
general population can accurately report how many people they know in
the hidden population and that all members of the hidden group are
equally likely to be known. These assumptions often fail in practice due
to stigma, recall issues, or social barriers that affect how people
perceive and report their connections. The MBSU addresses this by
introducing adjustment factors that correct for differential visibility
and reporting behavior.

Two primary adjustments are central to the MBSU model. First, the
visibility adjustment accounts for the fact that not all members of the
hidden population are equally likely to be known by others; it is
estimated using data from the hidden population itself, who report how
many people are aware of their membership in the group. Second, the
reporting adjustment corrects for the tendency of general population
respondents to underreport their known connections to hidden population
members, often due to stigma or social desirability bias. This
adjustment is typically estimated by comparing reporting accuracy across
groups with known sizes. By incorporating these two corrections, the
MBSU provides more accurate estimates of hidden population sizes under
realistic social conditions, especially when the assumptions of the
basic scale-up model are violated.
...
RDS uses ego-based information. That is, information about survey
respondent's attributes and characteristics. Each participant reports
the size of their personal network of eligible individuals, and these
degree reports are combined with the wave at which respondents were
recruited to adjust for unequal inclusion probabilities. The underlying
logic is that individuals with larger networks are more likely to be
recruited earlier and more often, creating a bias toward highly
connected respondents. However, is is not a simple random sample and so
we must account for non-random selection into the survey. RDS
...
many people they know with a given trait (for example, ``Do you know
personally of any other domesticc workers that are currently
experiencing any restrictions on their freedom? If so, how many?'').
...
albeit with significant variation in intensity. Conceptually, the
presumes that everyone in the sampling frame are exposed to some degree
of LE or risk of exploitation. Emperically this is all domestic workers
in the UK with a mobile phone (during the survey dates).
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-ma-risk-estimates-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-ma-binary-forest-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-rds-forest-plot-1.pdf}
...
<<<<<<< HEAD
\textcite{feeh16-generalizing} with Sequential Sampling weights and an
=======
\textcite{fee16-generalizing} with Sequential Sampling weights and an
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
estimated UK domestic worker population of 980,000, we find substantial
variation in prevalence estimates depending on the adjustment factors
applied (Table~\ref{tbl-nsum-main}).

The unadjusted MBSU estimates (baseline scenario with no adjustment for
<<<<<<< HEAD
network degree differences or visibility/reporting barriers, δ=1.0,
τ=1.0) suggest that document withholding affects approximately 8,120
workers (0.83\%, 95\% CI: 0.23\%-2.39\%). Pay-related issues affect
21,560 workers (2.20\%, 95\% CI: 1.45\%-4.35\%), while threats and abuse
affect 20,141 workers (2.06\%, 95\% CI: 1.42\%-4.48\%). Excessive
working hours affect 22,375 workers (2.28\%, 95\% CI: 1.41\%-4.21\%),
and 15,212 workers report lacking access to help (1.55\%, 95\% CI:
0.51\%-3.73\%).

Applying moderate adjustments for network degree differences and
visibility biases (δ=0.9 for degree ratio, τ=0.85 for
visibility/reporting) yields estimates of 10,615 workers experiencing
...
The most conservative adjustments (δ=0.8 for degree ratio, τ=0.7 for
visibility/transmission), accounting for greater network degree
differences and visibility barriers, produce substantially higher
...
The most conservative visability and reporting adjustments (δ
\(\delta=0.8\), τ\(\tau=0.7\)), accounting for greater underreporting
and visibility barriers, produce substantially higher estimates: 14,500
workers with document withholding (1.48\%, 95\% CI: 0.40\%-4.27\%),
38,499 with pay issues (3.93\%, 95\% CI: 2.60\%-7.77\%), 35,966
experiencing threats/abuse (3.67\%, 95\% CI: 2.53\%-8.00\%), 39,955 with
excessive hours (4.08\%, 95\% CI: 2.52\%-7.53\%), and 27,165 lacking
access to help (2.77\%, 95\% CI: 0.91\%-6.67\%).
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
...
=======
domestic workers. Adjustment factors: δ \(\delta\) (transmission/barrier
effect) and τ \(\tau\)(recall effect) control for visibility and
reporting biases. Bootstrap confidence intervals based on neighborhood
resampling (B=1000). Source: Authors' Own Work.}
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
...
<<<<<<< HEAD
differ across RDS and NSUM (reflecting differences in ego- versus
alter-based assumptions), the overall pattern is remarkably stable.

Figure~\ref{fig-method-comparison} demonstrates the comparison between
frequentist RDS estimators (RDS-I, RDS-SS) and Bayesian model-assisted
methods across all exploitation indicators. The horizontal spacing of
estimates allows clear visual comparison of point estimates and
uncertainty intervals, with RDS methods showing bootstrap confidence
intervals and Bayesian methods showing credible intervals from MCMC
estimation. Both approaches identify similar patterns of exploitation
prevalence, with excessive hours showing the highest prevalence across
all methods, followed by threats/abuse and pay-related issues.
=======
...
\includegraphics[width=12in,height=\textheight,keepaspectratio]{../output/figures/method_comparison.png}
...
<<<<<<< HEAD
\subsubsection{On the Role of Visibility in Explaining Differences
Between NSUM and RDS
=======
\subsection{On the Role of Visibility in Explaining Differences Between
NSUM and RDS
>>>>>>> b8fb866286418bc4845d972050dd8a343dd24956
...
\pandocbounded{\includegraphics[keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-demographics-1.pdf}}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-rds-sensitivity-popsize-1.pdf}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-ma-sensitivity-by-indicator-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012_files/figure-pdf/fig-ma-sensitivity-comparison-1.pdf}
...
Baraﬀ et al.~(2016)'s approach relies on a series of simplifying
assump5ons, namely: i) the social network is finite and connected ii)
network connec5ons are reciprocal, not directed iii) recruits accurately
report their network degree {[}this is facilitated by the concrete
request to provide a count of domes5c workers contacts within their
mobile phone -- thereby minimising recall error{[} iv) recruitment
coupons are distributed uniformly at random to neighbours in the network
v) that individuals may be recruited into the sample more than once.
Baraﬀ et al., note that this laLer assump5on par5cularly does not relate
to the reality of RDS in prac5ce.
=======
