...
#| fig-cap: "RDS Network Structure: Recruitment chains showing seed nodes (large circles) and recruitment waves. Node colors represent  nationality clusters, with node size proportional to reported network  degree (Q13). Lines show recruiter-recruit relationships."
#| fig-width: 12
#| fig-height: 8
#| echo: false
#| warning: false
...
data_viz <- prepared_data %>%
  dplyr::mutate(
    nationality_cluster = dplyr::case_when(
      ...
      stringr::str_detect(q8_a, "Asian|Indian") ~ "Asian",
...
cat("\n=== RDS Network Summary ===\n")
cat("Total sample size:", nrow(data_viz), "\n")
cat("Number of seeds:", sum(data_viz$is_seed), "\n")
cat("Number of referrals:", sum(!data_viz$is_seed), "\n")
cat("Number of waves:", max(data_viz$wave, na.rm = TRUE), "\n")
cat("Nationality distribution:\n")
print(table(data_viz$nationality_cluster, useNA = "ifany"))
cat("\nWave distribution:\n")
print(table(data_viz$wave, useNA = "ifany"))
```
...
The RDS estimates, calculated using RDS-II and Gile’s successive sampling estimator, suggest that overall prevalence of exploitation in the population is X percent (95 percent CI: X–Y). Subgroup analysis indicates that Latinx workers experience the highest prevalence (X percent), followed by Filipino workers (X percent), with British workers reporting markedly lower levels (X percent).
