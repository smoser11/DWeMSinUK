...
if (knitr::is_latex_output()) {
  # PDF build: safer options (no pack_rows or striped)
  knitr::kable(display_table,
               caption = "Clustered SS-PSE prevalence estimates by nationality subgroup and exploitation indicator",
               format = "latex",
               booktabs = TRUE, longtable = TRUE, linesep = "",
               align = c("l", "l", "r", "r")) %>%
    kableExtra::kable_styling(latex_options = c("hold_position"))
} else {
  # HTML & DOCX build: allow striped rows, scale down, and pack_rows
  knitr::kable(display_table,
               caption = "Clustered SS-PSE prevalence estimates by nationality subgroup and exploitation indicator",
               booktabs = TRUE, linesep = "",
               align = c("l", "l", "r", "r")) %>%
    kableExtra::kable_styling(latex_options = c("striped", "scale_down", "hold_position")) %>%
    kableExtra::pack_rows("Document withholding", 1, 4) %>%
    kableExtra::pack_rows("Pay-related issues", 5, 8) %>%
    kableExtra::pack_rows("Threats/abuse", 9, 12) %>%
    kableExtra::pack_rows("Excessive hours", 13, 16) %>%
    kableExtra::pack_rows("Limited access to help", 17, 20) %>%
    kableExtra::footnote(
      general = "Estimates based on Clustered Sequential Sampling Population Size Estimation methodology using netclust package. Population size assumption: 980,000 UK domestic workers. Confidence intervals derived from MCMC sampling (1,000 samples, 2,000 burn-in). Analysis accounts for nationality clustering (Filipino=62, Latinx=18, Other Combined=5) and network structure in RDS data.",
      general_title = "Note: ",
      escape = FALSE
    )
}
