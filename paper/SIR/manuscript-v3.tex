...
\makeatother
\subtitle{Manuscript v3 (2026-06-25) --- Items 1--10 of
critical-comments revision pass applied}
...
Section 3.2, Table M for the indicator-to-category mapping; ESM\_1 Table
...
framework for our measurement strategy. Table M (below) maps each ILO
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0451}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.3008}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1128}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.2030}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1579}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1278}}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0150}}@{}}
\caption{Table M. Mapping of the eleven ILO Indicators of Forced Labour
to this paper's measurement scheme. The five binary prevalence
indicators (right-hand grouping in column 4: document withholding,
threats and abuse, pay-related issues, limited access to help, excessive
working hours) jointly cover nine of the eleven ILO indicators (all
except abuse of vulnerability and deception, which are captured in the
composite only). The composite risk index covers all eleven ILO
indicators through eleven weighted sub-categories. Two further
evidentiary markers --- NRM-referral history (Q80) and payment below the
national living wage (Q36) --- are reported as separate binary flags and
are not part of the composite. Full per-category weight values are given
in ESM\_1, Table 1. Source: Authors' construction following ILO (2012,
2017).}\label{tbl-indicator-mapping}\tabularnewline
\toprule\noalign{}
\endfirsthead
\endhead
\bottomrule\noalign{}
...
:===: 1} & \multirow{2}{=}{ILO indicator (2012, 2017)

:====================================== Abuse of vulnerability} &
\multirow{2}{=}{ILO severity

:============: Medium} & \multirow{2}{=}{Binary prevalence indicator
(Section 4.3)

:========================= --- (composite only)} &
\multirow{2}{=}{Composite index (Section 4.2, Category)
:==================: Yes (Cat 1)} & \multirow{2}{=}{Survey item(s)

:==============: Q29, Q51} & \\
& & & & & & \\
2 & Deception & Medium & --- (composite only) & Yes (Cat 2) & Q45 & \\
3 & Restriction of movement & Medium & Limited access to help & Yes (Cat
3) & Q32, Q46 & \\
4 & Isolation & Medium & Limited access to help & Yes (Cat 4) & Q65 & \\
5 & Physical and sexual violence & Strong & Threats and abuse & Yes (Cat
5) &
\multicolumn{2}{>{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1429} + 2\tabcolsep}@{}}{%
Q44 (5c1, 5c1-b)} \\
6 & Intimidation and threats & Strong & Threats and abuse & Yes (Cat 6)
& Q47 & \\
7 & Retention of identity documents & Strong & Document withholding &
Yes (Cat 7) & Q70 & \\
8 & Withholding of wages & Medium & Pay-related issues & Yes (Cat 8) &
Q42 & \\
9 & Debt bondage & Medium & Pay-related issues & Yes (Cat 9) & Q39 & \\
10 & Abusive working and living conditions & Medium & Partial (Q48 feeds
Threats and abuse) & Yes (Cat 10) & Q48, Q63, Q72, Q76 & \\
11 & Excessive overtime & Medium & Excessive working hours & Yes (Cat
11) & Q16, Q61, Q62 & \\
\end{longtable}
...
\caption{\label{fig-recruitment}Figure 1. RDS Recruitment Network
Structure. Recruitment chains showing seed nodes (triangles) and recruit
nodes (circles). Node colours represent nationality clusters; node size
is proportional to network degree. Lines show recruiter-recruit
...
\caption{Table 1. Sample composition by nationality cluster and
recruitment wave (n = 85). Wave 0 indicates seed respondents; subsequent
waves are respondent-driven recruits. Filipino respondents dominate the
sample (76\%) and contribute the deepest recruitment chains; other
nationality clusters are smaller and shallower. Source: Authors' own
...
Before describing each estimator in turn, Table E summarises the
...
\caption{Table E. Estimands and identification vulnerabilities of the
four estimators applied in this paper. Each row characterises one
estimator: what information from the survey it uses, what population
quantity it targets, and what the principal threat to identification is.
Source: Authors' construction.}\label{tbl-estimands}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Method
...
in Table~\ref{tbl-estimands} above.
...
E), so absolute levels also differ by estimator in interpretable ways.

\subsection{4.1 Descriptive Overview}\label{descriptive-overview}

Sample characteristics are presented in Table~\ref{tbl-sample-chars} and
...
analytical sample is 93\% transnational migrant, 97\% female, 76\%
Filipino, with over half aged 45+ and 45\% self-employed.
...
\caption{Table D. RDS sampling diagnostics for the analytical sample (n
= 85). Computed directly from \texttt{data/processed/data\_nonzero.csv};
see Section 4.1.1 below and ESM\_4 (convergence plots per indicator) for
...
\caption{\label{fig-rds-diagnostics}Figure D. RDS sampling-design
diagnostics. Left: distribution of respondents by recruitment wave;
majority are seeds (wave 0) with only 6 of 85 respondents reaching wave
3 or higher. Middle: distribution of recruitment-chain lengths by root
seed; 30 of 46 chains are singletons (seed with no recruits), and one
chain (seed \#55, length 12) accounts for a disproportionate share of
wave-2+ respondents. Right: distribution of reported network size (q13),
showing strong right skew (median 5, mean 8.9, max 105). Source:
Authors' construction from \texttt{data/processed/data\_nonzero.csv}.}
...
composite (Section 3.2, Table M; full per-category weights in ESM\_1,
...
\caption{\label{fig-composite-risk}Figure 2. Composite Risk Index:
Comparison of Three Estimators. Mean composite risk index across the
observed RDS sample (descriptive; n=85), the RDS-SS adjusted estimator
(Gile's Successive Sampling with bootstrap 95\% CI), and the Bayesian
Model-Assisted (MA) estimator (with 95\% credible interval). The wide MA
credible interval reflects known limitations of model-assisted inference
for continuous outcomes; the RDS-SS estimate is the more reliable
population-adjusted figure. Source: Authors' own work.}
...
\textbf{?@tbl-preferred-estimates} reports the preferred prevalence
...
Moderate (alter-based) estimates side by side.
\textbf{?@fig-rds-methods} shows the RDS-SS estimates with per-method
bootstrap CIs for visual comparison with the RDS-I estimator (the
preferred binary-indicator estimator is RDS-SS).
...
\caption{\label{fig-ma-binary}Figure 3. Model-Assisted Estimates of
Exploitation Prevalence. Population prevalence estimates with 95\%
credible intervals for each exploitation indicator, ordered by
prevalence magnitude. Posterior intervals are from Bayesian
Model-Assisted inference (Gile \& Handcock, 2015) at N=50,000 with
seed.selection=`degree'. Source: Authors' own work.}
...
replicate the RDS-SS ordering reported in
\textbf{?@tbl-preferred-estimates}. (b) Direct comparison of RDS-I and
RDS-SS estimators for each indicator with per-method bootstrap 95\% CIs
(ESM\_4 Table 4); RDS-I runs systematically higher than RDS-SS by 5--15
percentage points, consistent with the standard finding that RDS-I is
sensitive to seed dependence and performs poorly at the small sample
sizes and shallow chain depths characteristic of our design (Section
4.1.1). (c) NSUM under the three discrete adjustment-factor tiers
(Unadjusted, Moderate, Conservative; ESM\_4 Table 5), presented for
direct comparison with the discrete-tier NSUM literature. The
substantive headline NSUM result is the continuous-\(\tau\) sweep
...
population quantity (Section 3.4, Table~\ref{tbl-estimands}). Second, we
...
and an eleven-indicator formative additive composite risk index for
ordinal severity gradation.
...
versus network-visible prevalence, Section 3.4 Table E), differences in
...
Reporting RDS and NSUM side-by-side (\textbf{?@tbl-preferred-estimates})
is the paper's design choice precisely because the gap is interpretable
as diagnostic evidence rather than as a single estimate of a single
quantity.
...
3.2, Table~\ref{tbl-indicator-mapping}). The remaining two --- abuse of
vulnerability and deception --- require retrospective interpretation
that brief-survey self-report cannot reliably elicit at \(n = 85\); both
are retained in the composite risk index (ESM\_1, Categories 1 and 2).
The paper's claim is therefore not ``prevalence of forced labour as
defined by the eleven ILO indicators'' but ``prevalence of nine of the
eleven as direct binary measures, with the remaining two captured
through the composite only''.
...
\textbf{Funding.} {[}To be completed: funding sources, grant numbers, or
``The authors received no specific funding for this work.''{]}

\textbf{Competing interests.} {[}To be completed: disclose any competing
financial or non-financial interests, or ``The authors have no competing
interests to declare.''{]}

\textbf{Ethics approval and consent to participate.} {[}To be completed:
ethics committee name and reference number, consent procedures. Note:
the manuscript currently refers to ``the lead author's School Research
Ethics Committee'' in Section 3 --- this language requires anonymisation
...
mirror: {[}OSF URL to be inserted on submission{]}. The underlying
GitHub repository is currently private and will be made public on
acceptance.
...
\textbf{Authors' contributions.} {[}To be completed.{]}
...
: Table 2. Preferred Prevalence Estimates for the Five Binary
ILO-Aligned Indicators (UK Domestic Workers, N = 980,000). RDS-SS
columns from neighbourhood-bootstrap 95\% CIs (B = 1000); MBSU NSUM
(Moderate: \(\delta = 0.9\), \(\tau = 0.85\)) columns from
neighbourhood-bootstrap 95\% CIs. Indicators ordered from highest to
lowest ILO severity (strong \(\rightarrow\) medium). Source: Authors'
own work. \{\#tbl-preferred-estimates\}
