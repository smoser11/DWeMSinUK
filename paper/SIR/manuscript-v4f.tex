...
(\textbf{?@sec-estimation-framework}). The preferred NSUM result is the
continuous-\(\tau\) sweep over \(\tau \in [0.40, 1.00]\) with \(\delta\)
fixed at 0.9 (\textbf{?@sec-estimation-framework}; ESM\_4
...
(\textbf{?@sec-estimation-framework}, Table~\ref{tbl-estimands}), so
absolute levels also differ by estimator in interpretable ways.

\subsection{Descriptive Overview}\label{sec-descriptive}

Sample characteristics are presented in \textbf{?@tbl-sample-chars} and
...
as noted in \textbf{?@sec-estimation-framework}, MA-RDS does not
...
population quantity (\textbf{?@sec-estimation-framework},
