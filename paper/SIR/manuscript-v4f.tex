...
\addbibresource{ce-library.bib}
\addbibresource{sm-references.bib}
...
\makeatother
\subtitle{Manuscript v4e (2026-06-25) --- citations cleaned, word trim,
Quarto cross-refs restored}
...
prevelance of different forms of laour exploitation are robust across a
number of paramters. The paper contributes a replicable measurement
...
\autocite{ILO11-indicators,international_labour_organization_global_2017}
provide an operational framework for measuring modern slavery, but the
empirical question of which indicators are observable through which
survey design remains open. Studies typically apply one design ---
household survey, respondent-driven sampling (RDS), or the network
scale-up method (NSUM) --- and report prevalence accordingly. None to
our knowledge applies both RDS and NSUM to the same survey instrument
for the same indicators, the design that would allow direct
triangulation of ego-based against alter-based estimates and, where the
two diverge, diagnostic inference about which exploitation behaviours
are visible within personal networks and which are not.
...
organised around the ILO's operational distinction
\autocite{ILO11-indicators,international_labour_organization_global_2017}
...
labour
\autocite{ILO11-indicators,international_labour_organization_global_2017}
provide the canonical operational framework for measuring modern
slavery. The ILO distinguishes two grades of indicator: `medium'
indicators (excessive overtime, wage withholding, debt bondage,
...
and passport withholding. The remaining two ILO indicators --- abuse of
...
included in the composite risk index
(Section~\ref{sec-composite-results}; ESM\_1
Table~\ref{tbl-indicator-mapping}, Categories 1 and 2). Where
...
ESM\_1 Table~\ref{tbl-indicator-mapping} for per-category weights). It
is constructed as a formative additive composite: an individual is
exposed to exploitation if any of the indicators is present, and the
indicators are constitutive rather than reflective of an underlying
severity. Polarity is uniform across categories. Weights are anchored on
the ILO medium/strong severity gradation --- strong-indicator categories
receive larger sub-weights than medium-indicator categories --- and we
conduct robustness assessment by perturbing the weights
...
\subsection{2.1 Evaluating the Degree of Risk}\label{sec-degree-of-risk}
...
eleven ILO indicators (Section~\ref{sec-composite-results}; ESM\_1 for
full construction).
...
\autocite{kala08-new,mant16-modern,serv23-closed,yilm23-exploring}, but
the qualitative and case-based evidence base has not yielded
...
workplaces, and civil-society-mediated access (undocumented agricultural
workers, kafala-system migrants in Gulf states, informal-construction
day labourers).

\subsection{3.1 Study Design and Sampling}\label{sec-study-design}
...
Recruits, in turn, recruit others, creating waves of participation. Each
participant reports their network degree (the number of eligible
contacts they know), which is used to adjust for selection
...
\autocite{ILO11-indicators}. The eleven ILO indicators of forced labour
\autocite{ILO11-indicators,international_labour_organization_global_2017}
provide the operational framework for our measurement strategy.
...
composite. Full per-category weight values are given in ESM\_1,
Table~\ref{tbl-indicator-mapping}. Source: Authors' construction
following ILO (2012, 2017).}\label{tbl-indicator-mapping}\tabularnewline
\toprule\noalign{}
...
\autocite{gile11-improved,gile18-methods} provide valid population
estimates even for modest sample sizes, particularly when uncertainty is
appropriately quantified through resampling or posterior distributions.
In this study, we additionally employ a neighbourhood bootstrap
procedure that propagates sampling and network uncertainty, providing
robust confidence intervals consistent with small-sample design-based
inference \autocite{hand14-estimating}.
...
Table~\ref{tbl-indicator-mapping}).
...
\subsection{3.4 Estimation Framework}\label{sec-estimation-framework}
...
single underlying quantity (see Section~\ref{sec-divergence} for the
diagnostic interpretation).

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 0\tabcolsep) * \real{1.0069}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
Method \textbar{} Information used \textbar{} Estimand \textbar{} Main
vulnerability \textbar{}
:====================+:===============================+:===================================+:=====================================+
RDS-SS \textbar{} Respondent's own experience \textbar{} Own-experience
prevalence among \textbar{} Seed dependence, shallow \textbar{}
(Section~\ref{sec-binary-results}) \textbar{} + reported degree +
\textbar{} the RDS-reachable population of \textbar{} recruitment
chains, homophily, \textbar{} \textbar{} recruitment-chain structure
\textbar{} UK migrant domestic workers \textbar{} unobserved degree
heterogeneity \textbar{} \\
MA-RDS \textbar{} Same as RDS-SS, plus a working \textbar{}
Model-assisted own-experience \textbar{} Network-model misspecification,
\textbar{} (Section~\ref{sec-supporting-estimates}, \textbar{}
Exponential Random Graph \textbar{} prevalence (same target as RDS-SS
\textbar{} convergence limits at small \(n\); supporting) \textbar{}
network model conditional on \textbar{} but with explicit network
\textbar{} does not reliably converge for \textbar{} \textbar{} seeds,
degree, and homophily \textbar{} structure adjustment) \textbar{}
continuous outcomes \textbar{} \\
NSUM (MBSU) \textbar{} Respondent reports about \textbar{}
Network-visible prevalence among \textbar{} Transmission bias,
visibility \textbar{} (Section~\ref{sec-binary-results}, \textbar{}
alters in their personal \textbar{} domestic-worker alters in the
\textbar{} gradient across exploitation \textbar{} preferred) \textbar{}
network \textbar{} respondents' aggregated network \textbar{}
behaviours, alter misclassification, \textbar{} \textbar{} \textbar{}
\textbar{} unknown true-positive rate (\(\tau\)) \textbar{} \\
Composite risk \textbar{} Respondent's own indicator \textbar{}
RDS-adjusted population mean of \textbar{} Weight specification,
construct \textbar{} index (Section~\ref{sec-composite-results})
\textbar{} profile across all eleven \textbar{} the formative-additive
severity \textbar{} validity, formative-vs-reflective \textbar{}
\textbar{} ILO indicators \textbar{} score \textbar{} measurement
assumptions \textbar{} \\
\end{longtable}
...
implementation in the RDS package (MA.estimates) is designed primarily
for binary traits, and we find that for the continuous composite risk
index it does not converge to a useful posterior at our sample size
(n=85); the credible interval covers essentially the full support of the
variable. We therefore report frequentist RDS-SS estimates as the
headline for the composite index and reserve MA-RDS estimates for the
five binary indicators (see Section~\ref{sec-composite-results} and
ESM\_4).

\emph{Within-Population Scale-Up Estimation (NSUM):} We employ the
Modified Basic Scale-Up (MBSU) model of \textcite{feeh16-generalizing}
in a non-standard configuration: respondents are themselves members of
the hidden population, reporting on other UK migrant domestic workers
they know. We treat this as a \emph{within-population scale-up} design
\autocite[supplementary materials]{feeh16-generalizing} or
...
visibility-gradient diagnostic (Section~\ref{sec-divergence}).
...
CIs at each value (ESM\_4 Figure~\ref{fig-recruitment}). For comparison
with the discrete-tier NSUM literature
\autocite{kill98-social,malt15-estimating,feeh16-generalizing}, we
report three reference scenarios --- Unadjusted
(\(\delta = 1.0, \tau = 1.0\)), Moderate
...
Procedures}\label{sec-bootstrap}
...
adjustment factor values. Full results are reported in Online Resource
ESM\_4.

\section{Results}\label{sec-results}

We present results in five subsections. Section~\ref{sec-descriptive}
describes the analytical sample. Sections 4.2 and 4.3 report the
preferred prevalence estimates for the continuous composite risk index
and the five binary ILO-aligned indicators respectively. For binary
indicators, the preferred estimator is RDS-SS (Gile's Successive
...
\(\delta\) fixed at 0.9 (Section~\ref{sec-estimation-framework}; ESM\_4
Figure~\ref{fig-recruitment}); the Moderate discrete-tier point estimate
(\(\delta = 0.9\), \(\tau = 0.85\)) is reported alongside as a reference
scenario for direct comparison with the discrete-tier NSUM literature.
...
estimators target different population quantities
(Section~\ref{sec-estimation-framework}, Table~\ref{tbl-estimands}), so
absolute levels also differ by estimator in interpretable ways.

\subsection{4.1 Descriptive Overview}\label{sec-descriptive}
...
Diagnostics}\label{sec-rds-diagnostics}
...
Section~\ref{sec-rds-diagnostics} below and ESM\_4 (convergence plots
per indicator) for the full set. Source: Authors'
...
skew (median 5, mean 8.9, max 105). Source: Authors' construction from
...
comparison reported in Section~\ref{sec-supporting-estimates} quantifies
how much the standard equilibrium assumption matters (RDS-I runs 5--15
percentage points higher, with the gap larger where seed-cluster
homophily is strongest).
...
composite (Section~\ref{sec-survey-instrument},
Table~\ref{tbl-indicator-mapping}; full per-category weights in ESM\_1,
Table~\ref{tbl-indicator-mapping}). Rather than treating exploitation as
a dichotomy, the risk index conceptualises every domestic worker as
facing some degree of potential exploitation, varying continuously in
...
for completeness Figure~\ref{fig-rds-diagnostics} shows the MA estimate
...
Moderate (alter-based) estimates side by side. ESM\_4 Figure B.1 shows
the RDS-SS estimates with per-method bootstrap CIs for visual comparison
with the RDS-I estimator (the preferred binary-indicator estimator is
RDS-SS).
...
(ESM\_4 Table~\ref{tbl-estimands}); posterior means range from 34.1\%
(document withholding) to 84.0\% (excessive working hours) and
substantively replicate the RDS-SS ordering reported in
...
(ESM\_4 Table~\ref{tbl-rds-diagnostics}); RDS-I runs systematically
higher than RDS-SS by 5--15 percentage points, consistent with the
standard finding that RDS-I is sensitive to seed dependence and performs
poorly at the small sample sizes and shallow chain depths characteristic
of our design (Section~\ref{sec-rds-diagnostics}). (c) NSUM under the
three discrete adjustment-factor tiers (Unadjusted, Moderate,
Conservative; ESM\_4 Table~\ref{tbl-preferred-estimates}), presented for
...
\subsection{4.5 Sensitivity and Robustness}\label{sec-sensitivity}

Three sensitivity analyses are reported in full in ESM\_4. First, the
continuous-\(\tau\) NSUM sweep (ESM\_4 Figure~\ref{fig-recruitment})
shows MBSU prevalence monotonically increasing as \(\tau\) decreases;
the increase is modest across the plausible range
\(\tau \in [0.7, 1.0]\) and substantial only at low \(\tau \le 0.5\).
This continuous picture supersedes the discrete-tier comparison
(Section~\ref{sec-supporting-estimates}) for substantive inference about
how visibility assumptions affect estimates.

Second, the MA-RDS seed-selection sensitivity (ESM\_4
Figure~\ref{fig-rds-diagnostics}) shows that the Bayesian MA posterior
estimates are insensitive to the choice of seed-selection method
(degree, random, sample) at our sample size --- different chains
converge to the same posterior, confirming model robustness to
initialisation.
...
\subsection{5.1 Summary of Findings}\label{sec-summary}
...
complementary validation channels. (1) \emph{Content validity:} the
indicator set is anchored in the ILO
\autocite*{ILO11-indicators,international_labour_organization_global_2017}
framework of forced labour, with the mapping from indicator to survey
question documented in Section~\ref{sec-survey-instrument}
(Table~\ref{tbl-indicator-mapping}) and ESM\_1. (2) \emph{Face
validity:} the survey instrument was co-developed with the participating
civil-society organisations (Kalayaan and the Latin American Women's
Rights Service), whose case-work practice informed item wording and
indicator coverage. (3) \emph{Convergent and diagnostic validity:} the
four estimators target related-but-distinct population quantities and
nonetheless reproduce the same within-method ordering of indicators
across methodologically unrelated approaches
(Section~\ref{sec-sensitivity}); the order-of-magnitude divergence in
absolute level between RDS and NSUM, by contrast, has diagnostic rather
than convergent value, telling us about the visibility structure of each
behaviour (Section~\ref{sec-divergence}). (4) \emph{Robustness:} the
composite-risk ranking is essentially weight-invariant under
\(\pm 50\%\) perturbation of the eleven sub-indicator weights
(Section~\ref{sec-weights-sensitivity}; Spearman \(\rho = 0.992\)), and
the headline NSUM estimates are reported as a continuous function of the
unknown true-positive rate \(\tau\) rather than committing to a single
point (Section~\ref{sec-sensitivity}). (5) \emph{Limitation:} the
strategy does not yet include external validation against administrative
data sources (NRM referrals, employment-tribunal records); we identify
this as the natural next step in the broader SDG 8.7 monitoring
architecture (Section~\ref{sec-policy}).
...
three conceptually distinct sources of dispersion, each carrying a
different epistemic status; and on the only one where sample size
matters, n = 85 is competitive for hidden-population designs.
...
severe forms of coercion (document confiscation, an ILO strong
indicator) are less common in this population than less severe forms of
abuse (excessive hours, ILO medium). The hierarchy is interpretable,
matches the ILO severity gradation, and is reproduced across methods.
Collapsing this hierarchy into a single ``wide range'' headline obscures
what is arguably the paper's most policy-relevant finding: different
exploitation behaviours have meaningfully different population
frequencies.
...
the continuous-\(\tau\) sweep in ESM\_4 makes explicit). Reporting RDS
and NSUM side-by-side (Table~\ref{tbl-preferred-estimates}) is the
...
continuous-\(\tau\) sweep in ESM\_4 rather than as a single point.
Sample size operates through this third channel and only this third
channel: more respondents would narrow the bootstrap CIs but would not
narrow the across-indicator hierarchy (which is real) or the
...
prevalences, jointly covering nine of the eleven ILO indicators
(Section~\ref{sec-survey-instrument},
Table~\ref{tbl-indicator-mapping}). The remaining two --- abuse of
...
Section~\ref{sec-survey-instrument} and ESM\_1). We address this with a
weight-perturbation sensitivity analysis: each weight was multiplied by
an independent uniform factor in \([0.5, 1.5]\) (i.e., \(\pm 50\%\)
perturbation), the composite was recomputed under the perturbed weights,
and the resulting respondent ranking was compared to baseline. We
repeated this for 5,000 independent perturbations.
...
--- is replicable for other hidden populations relevant to 8.7
monitoring (undocumented agricultural workers, sex workers,
kafala-system migrants). The specific numerical estimates here apply
only to UK migrant domestic workers; the design is the contribution.
...
formative additive composite risk index anchored on the ILO
medium/strong severity hierarchy.
