...
--- and report prevalence accordingly. None to our knowledge applies
both RDS and NSUM to the same survey instrument for the same indicators,
the design that would allow direct triangulation of ego-based against
alter-based estimates and, where the two diverge, diagnostic inference
about which exploitation behaviours are visible within personal networks
and which are not.
...
The ILO's eleven indicators of forced labour
\autocite{ILO11-indicators,int25-ilo} provide the operational anchor,
and the ILO's survey-guidelines handbook \autocite{hard24-ilo}
distinguishes `strong' indicators (retention of identity documents,
intimidation and threats, physical and sexual violence --- any one of
which is generally sufficient on its own to identify forced labour in
survey settings) from `medium' indicators (multiple typically required).
This severity ordering lets a single survey instrument capture
exploitation across the full range and supports a continuous formative
...
given in Appendix Table A1. Source: Authors' construction following ILO
...
\autocite*{ILO11-indicators,hard24-ilo,int25-ilo} framework of forced
labour, with the mapping from indicator to survey question documented in
Section~\ref{sec-survey-instrument} (Table~\ref{tbl-indicator-mapping})
and the Appendix (Section A). (2) \emph{Face validity:} the survey
instrument was co-developed with the participating civil-society
organisations (Kalayaan, Voice of Domestic Workers, and the Latin
...
in \autocite{hard24-ilo}) are less common in this population than less
