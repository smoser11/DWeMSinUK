...
conceptualises labour exploitation as a graded formative indicator
framework measured by five ILO-aligned indicators arranged
hierarchically by severity. Section 3 describes the matched-instrument
survey design (Section 3.1) and the dual sampling approach combining RDS
and NSUM (Section 3.2). Section 4 presents prevalence estimates by
indicator and method, including frequentist RDS-I/RDS-II/RDS-SS
estimators (Section 4.1), Bayesian Model-Assisted inference (Section
4.2), and NSUM with continuous-\(\tau\) sensitivity analysis (Section
4.3). Section 5 reads the RDS-NSUM divergence as diagnostic evidence
about visibility within workers' personal networks and discusses
implications for SDG 8.7 monitoring.
...
--- each indicator has the same direction with respect to the underlying
formative composite of exploitation severity. The choice of aggregation
follows the guidance of Maggino (2017) and the SIR editorial team
(Bartram et al.~2024): we use a weighted additive index in which weights
are anchored on the ILO medium/strong severity gradation
(strong-indicator categories receive larger sub-weights than
medium-indicator categories), and we conduct robustness assessment by
perturbing the weights (Section 5.5). Two further evidentiary markers
--- NRM-referral history and payment below the national living wage ---
are reported as separate binary flags and are not part of the composite,
as their direct evidentiary status under UK policy makes them more
useful read independently of the composite construction.
...
modern slavery as a graded formative indicator framework rather than a
...
Section 3.2 (Table 1) and ESM\_1. (2) \emph{Face validity:} the survey
instrument was co-developed with the participating civil-society
organisations (Kalayaan and the Latin American Women's Rights Service),
whose case-work practice informed item wording and indicator coverage.
(3) \emph{Convergent and diagnostic validity:} the four estimators
target related-but-distinct population quantities and nonetheless
reproduce the same within-method ordering of indicators across
methodologically unrelated approaches (Section 4.5); the
...
the visibility structure of each behaviour (Section 5.3). (4)
\emph{Robustness:} the composite-risk ranking is essentially
weight-invariant under \(\pm 50\%\) perturbation of the eleven
sub-indicator weights (Section 5.5; Spearman \(\rho = 0.992\)), and the
headline NSUM estimates are reported as a continuous function of the
unknown true-positive rate \(\tau\) rather than committing to a single
point (Section 4.5). (5) \emph{Limitation:} the strategy does not yet
include external validation against administrative data sources (NRM
referrals, employment-tribunal records); we identify this as the natural
next step in the broader SDG 8.7 monitoring architecture (Section 5.6).
...
\emph{different} population quantity (Table 3) --- own-experience
prevalence, network-visible prevalence, and severity score --- so no
single estimate is identified as `the prevalence of forced labour among
UK domestic workers'. If RDS over-samples respondents with first-hand
experience and NSUM under-detects concealed exploitation, the true
prevalence for a given indicator plausibly lies between the two reports,
with the precise location depending on assumptions about visibility,
sampling, and reporting that the data cannot pin down. We treat this as
a working hypothesis, not an identification result. The design does
licence: (a) a conservative reference scenario under the stated MBSU
assumptions (MBSU Moderate, \(\delta = 0.9\), \(\tau = 0.85\), on the
conservative side of the continuous-\(\tau\) sweep in Section 4.5 --- a
plausible lower-end benchmark, not an identified lower bound); (b) a
...
visibility diagnostic with direct implications for network-mediated
outreach (Section 5.6).
