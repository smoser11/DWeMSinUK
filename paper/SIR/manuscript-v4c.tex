...
licence is (a) a conservative reference scenario under the stated MBSU
assumptions (the MBSU Moderate figures, with \(\delta = 0.9\) and
\(\tau = 0.85\), sit on the conservative side of the continuous-\(\tau\)
sweep reported in Section 4.5; they are a plausible lower-end benchmark
conditional on those adjustment factors, not an identified lower bound);
(b) a credible characterisation of relative incidence among respondents
from the RDS estimates, and of the indicator severity hierarchy from the
cross-method robustness; and (c) a diagnostic of which exploitation
behaviours are visible to other workers and which are not --- a finding
with direct implications for the design of network-mediated outreach
interventions (Section 5.6).
