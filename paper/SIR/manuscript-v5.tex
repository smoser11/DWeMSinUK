...
(Section~\ref{sec-composite-results}; Appendix Table A1, Categories 1
and 2). Where appropriate we note this scope limitation in the
...
Appendix Table A1 for per-category weights). It is constructed as a
...
rather than reflective of an underlying severity. Polarity is uniform
across categories. Weights are anchored on the ILO medium/strong
severity gradation --- strong-indicator categories receive larger
sub-weights than medium-indicator categories --- and we conduct
robustness assessment by perturbing the weights
...
composite. Full per-category weight values are given in Appendix Table
A1. Source: Authors' construction following ILO (2012,
...
five binary indicators (see Section~\ref{sec-composite-results} and the
Appendix).
...
(Appendix Figure A1). For comparison with the discrete-tier NSUM
literature
...
\(\delta\) fixed at 0.9 (Section~\ref{sec-estimation-framework};
Appendix Figure A1); the Moderate discrete-tier point estimate
...
Table A1). Rather than treating exploitation as a dichotomy, the risk
...
Moderate (alter-based) estimates side by side. Appendix Figure A2 shows
...
Bayesian Model-Assisted (MA-RDS) posterior means (Appendix Table A6)
range from 34.1\% for document withholding to 84.0\% for excessive
working hours, reproducing the RDS-SS ordering in Table 5. (b) Direct
comparison of RDS-I and RDS-SS estimators for each indicator with
per-method bootstrap 95\% CIs (Appendix Table A4); RDS-I runs
...
performs poorly at the small sample sizes and shallow chain depths
characteristic of our design (Section~\ref{sec-rds-diagnostics}). (c)
NSUM under the three discrete adjustment-factor tiers (Unadjusted,
Moderate, Conservative; Appendix Table A10), presented for direct
comparison with the discrete-tier NSUM literature. The substantive
headline NSUM result is the continuous-\(\tau\) sweep reported in
...
the continuous-\(\tau\) NSUM sweep (Appendix Figure A1) shows MBSU
...
Second, the MA-RDS seed-selection sensitivity (Appendix Figure A4) shows
...
(Table~\ref{tbl-indicator-mapping}) and the Appendix (Section A). (2)
\emph{Face validity:} the survey instrument was co-developed with the
participating civil-society organisations (Kalayaan and the Latin
American Women's Rights Service), whose case-work practice informed item
wording and indicator coverage. (3) \emph{Convergent and diagnostic
validity:} the four estimators target related-but-distinct population
quantities and nonetheless reproduce the same within-method ordering of
indicators across methodologically unrelated approaches
...
continuous-\(\tau\) sweep in the Appendix (Section D.5, Figure A1)
...
are retained in the composite risk index (Appendix Table A1, Categories
1 and 2). The paper's claim is therefore not ``prevalence of forced
labour as defined by the eleven ILO indicators'' but ``prevalence of
nine of the eleven as direct binary measures, with the remaining two
captured through the composite only''.
