...
This section documents the composite risk index construction, the five binary exploitation indicators used as the primary prevalence outcomes, and the key RDS variables. Data and replication code are available at [OSF view-only URL: osf.io/vf5yb/]. The composite risk index and all indicator variables are constructed in `R/data_processing/02-data_preparation.R` via `create_comparable_indicators()`.

## Composite Risk Index

The composite risk index aggregates all eleven ILO indicators of forced labour into a single continuous measure (range 0–0.99). The index is a formative additive composite: an individual is exposed to exploitation if any of the indicators is present, and the indicators are constitutive rather than reflective of an underlying severity construct. Polarity is uniform across categories. Weights are anchored on the ILO medium/strong severity gradation: strong-indicator categories receive larger sub-weights than medium-indicator categories.
...
: Table A1. Composite Risk Index: ILO indicator categories, survey items, coding rules, and sub-weights. Strong-indicator categories receive sub-weight 0.09 per sub-category; medium-indicator categories receive 0.045 per sub-category (two sub-questions) or 0.03 (three sub-questions). The total composite score sums to a maximum of 0.99. Categories 12 and 13 are reported as separate binary flags and are excluded from the composite.
...
This section documents the mapping between ego-based (RDS) and alter-based (NSUM) survey questions for the five binary exploitation indicators. The ego-based questions ask respondents about their own experiences; the alter-based questions ask what respondents observe in the domestic workers they know. This ego/alter pairing is the foundation of the dual-method design.

: Table B1. Comparable Ego- and Alter-Based Indicator Pairs. Confidence ratings reflect the closeness of the conceptual match between the ego (RDS) and alter (NSUM) survey questions. All indicators are binary (0/1) after recoding.

| Indicator | RDS (ego) question(s) | NSUM (alter) question | Confidence |
|---|---|---|---|
| Document withholding | Q70 (5f2): Travel/identity documents withheld from respondent | Q71 (5f3): Others who lack access to their own documents | Highest |
| Pay/debt issues | Q39 (5b4): Earnings used to pay off debt to helper; Q42 (5b7): Pay ever withheld from respondent | Q43 (5b8): Others with debt or pay problems | High |
| Threats and abuse | Q45 (5c2): Forced/deceived/threatened into poor conditions; Q47 (5c4): Employer threatened or intimidated respondent; Q48 (5c5): Verbally abused | Q49 (5c6): Others who experienced threat or force | High |
| Excessive hours | Q61 (5d8): Weekly rest < 24h; Q62 (5d9): Excessive overtime felt | Q64 (5d11): Others working excessive overtime, lacking breaks or annual leave | Medium (Q64 also covers annual leave, not in RDS questions) |
| Access to help | Q78 (5f10): Knows where to get help (reverse-coded: no access → 1) | Q79 (5f11): Others who do NOT know where to get help | Lower |

**Data processing notes.** All indicators are converted to binary (0/1) before analysis. RDS multi-category responses are recoded: affirmative answers → 1, Never/no access → 0, "Don't know"/"Prefer not to say" → NA. RDS composite indicators (threats and abuse; pay issues; excessive hours) are combined using logical OR: any affirmative sub-question → 1. NSUM count variables (number of domestic workers known with the characteristic) are converted to binary: > 0 → 1. These constructions follow the comparable-indicator specification agreed in November 2024 (see `create_comparable_indicators()` in `R/data_processing/02-data_preparation.R`).
...
|---|---|---|
| MBSU No Adjustment (δ=1.0, τ=1.0) | Access to help | 1.55% (0.51–3.73%) |
| MBSU No Adjustment (δ=1.0, τ=1.0) | Document withholding | 0.83% (0.23–2.39%) |
| MBSU No Adjustment (δ=1.0, τ=1.0) | Excessive hours | 2.28% (1.41–4.21%) |
| MBSU No Adjustment (δ=1.0, τ=1.0) | Pay issues | 2.20% (1.45–4.35%) |
| MBSU No Adjustment (δ=1.0, τ=1.0) | Threats and abuse | 2.06% (1.42–4.48%) |
| MBSU Moderate (δ=0.9, τ=0.85) | Access to help | 2.03% (0.67–4.88%) |
| MBSU Moderate (δ=0.9, τ=0.85) | Document withholding | 1.08% (0.30–3.13%) |
| MBSU Moderate (δ=0.9, τ=0.85) | Excessive hours | 2.98% (1.84–5.51%) |
| MBSU Moderate (δ=0.9, τ=0.85) | Pay issues | 2.88% (1.90–5.68%) |
| MBSU Moderate (δ=0.9, τ=0.85) | Threats and abuse | 2.69% (1.86–5.86%) |
| MBSU Conservative (δ=0.8, τ=0.70) | Access to help | 2.77% (0.91–6.67%) |
| MBSU Conservative (δ=0.8, τ=0.70) | Document withholding | 1.48% (0.40–4.27%) |
| MBSU Conservative (δ=0.8, τ=0.70) | Excessive hours | 4.08% (2.52–7.53%) |
| MBSU Conservative (δ=0.8, τ=0.70) | Pay issues | 3.93% (2.60–7.77%) |
| MBSU Conservative (δ=0.8, τ=0.70) | Threats and abuse | 3.67% (2.53–8.00%) |
