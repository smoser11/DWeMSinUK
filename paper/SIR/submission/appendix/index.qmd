...
All data and replication code are available at [OSF]([OSF view-only URL redacted for anonymised review; will be provided upon request]). The composite risk index and all indicator variables are constructed in `R/data_processing/02-data_preparation.R` via `create_comparable_indicators()`.
