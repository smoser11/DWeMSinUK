...
of Modern Slavery Risk Among UK Domestic Workers}
\author{}
...
Sampling (RDS) methods RDS-I and RDS-SS and Network Scale Up (NSUM)
...
According to \autocite{bonnet_domestic_2022}, eighty percent of this domestic work is unregulated and informal. Labour
...
coupled with Network Scale-up Methods (NSUM) to reach and sample
...
First we discuss our study in more detail, highlighting what is already known about the
...
exploitation
\autocite{oconnell_davidson_margins_2015,gutierrez-huerter_o_change_2023}.
For this reason, we used the International Labour Organization's
\autocite{ILO11-indicators} `Indicators of Forced Labour' to identify
the potential for severe labour exploitation and as a basis for the
quantification of our labour exploitation and abuse risk index. The ILO
identify eleven indicators designed to help understand how forced labour
arises and how it affects victims. These indicators include: abuse of
vulnerability; deception; restriction of movement; isolation; physical
and sexual violence; intimidation and threats; retention of identity
documents; withholding of wages; debt bondage; abusive working and
living conditions and excessive overtime. According to the ILO, the
...
\subsubsection{Evaluating the Degree of
...
\autocite{tarei_hybrid_2018,dellana_scale_2021}, including the
quantification of a risk index for the petroleum supply chain
\autocite{tarei_hybrid_2018}. Yet, while these authors recognize the
need for responsible management and its effect on societal values, in
line with other literature in the field they view risk from the
perspective of the corporate supply chain rather than examining the risk
of harm to the worker.
...
conceptualization of labour exploitation (LE) as a continuous \emph{risk
index} (or risk score -- see Appendix A for details of its construction)
we also employ a series of dichotomous indicators.
...
Our approach, therefore, includes, but goes beyond, assessing the
likelihood of forced labour by simply quantifying the proportion of
survivors entering the UK's National Referral Mechanism (NRM): a
government system for survivor support set up to identify whether there
are positive grounds for the identification of Modern Slavery. In our
method, an NRM referral is used as the strongest indicator of modern
slavery risk, with lesser risks assessed according to the degree to
which cumulative indicators of forced labour are reported.
...
\textbf{CAROLINE: DO WE NEED THE FOLLOWING PARAGRAPH?} In 2023,
\textcite{strauss_britain_2023} reported a big shift in the source
countries of migrants arriving in the UK on the Overseas Domestic Worker
and other types of worker visas. Transnational domestic workers from the
Philippines and India accounted for the single largest number of
applications granted (10,186 and 3,858 visas respectively), followed by
smaller, but still significant, numbers of workers arriving from
...
modern slavery through the UK's NRM \autocite{romero_blueprint_2025}.
...
\autocite{heckathorn_respondent-driven_1997,heckathorn_comment_2011,gile_methods_2018}.
It combines chain-referral sampling with a statistical framework to
produce population estimates that adjust for network structure and
differential recruitment probabilities. The method begins with a set of
initial participants (``seeds''), who recruit peers from their social
networks. Recruits, in turn, recruit others, creating waves of
participation. Each participant reports their network degree (the number
of eligible contacts they know), which is used to adjust for selection
bias---especially the oversampling of high-degree individuals.

RDS has been widely used to sample hidden populations in public health
and social science research
\autocite{mccreesh_evaluation_2012,schwitters_prevalence_2012}, and
network-based referrals have been described as the only viable method to
reach many types of labour trafficking victims
\autocite{zhang_measuring_2012}. Recent applications include studies of
exploitation among low-wage workers in American cities
\autocite{bernhardt_broken_2009}, labour trafficking in migrant
communities \autocite{vincent_estimating_2017}, child labour in India
\autocite{zhang_victims_2019}, and commercial sexual exploitation in
Nepal \autocite{jordan_overcoming_2020}.

Our approach can best be described as Web-based RDS
\autocite{wejnert_web-based_2008}. We designed a web survey using the
JISC online survey interface, suitable for respondents to complete via
mobile phone. The main survey was conducted over five months between
February and July 2023.

\subsubsection{Seed Selection and
Recruitment}\label{seed-selection-and-recruitment}
...
\subsubsection{Incentive Design and Participation
Verification}\label{incentive-design-and-participation-verification}
...
workers within their existing social network, it was the up to five of
...
\textbf{Ego-based questions} capture respondents' own experiences of
exploitation (used for RDS estimation), while \textbf{alter-based
questions} ask respondents to report on the exploitation experiences of
domestic workers they know (used for NSUM estimation). See Appendix A
for details of these survey questions and the construction of comparable
...
respondents for RDS-based estimation.
...
such workers representing over half of the sample (see
Table~\ref{tbl-sample-characteristics}).

Figure~\ref{fig-rds-network} presents the recruitment network structure
for the 85 respondents included in RDS analysis.

\begin{figure}[H]

\centering{

\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-rds-network-1.pdf}
...
\subsection{Ethical Approval and Use of
AI}\label{ethical-approval-and-use-of-ai}
...
robustness and conduct sensitivity analyses (see Appendix B). The dual
...
We implemented three RDS estimators, each with distinct assumptions:
RDS-I \autocite{salg04-sampling}, the Volz--Heckathorn (VH) estimator
\autocite{volz08-probability}, and Gile's Successive Sampling (RDS-SS)
estimator \autocite{gile11-improved}. Given our small sample (85
respondents) and non-negligible sampling fraction, RDS-SS serves as our
primary estimator for binary outcomes because it accounts for finite
population effects. RDS-I, which performs poorly in small samples and is
highly sensitive to seed dependence, is retained as a historical
benchmark.
...
We employ the Modified Basic Scale-Up (MBSU) model
\autocite{feeh16-generalizing}, which improves upon the original Basic
Scale-Up Method by accounting for key sources of bias. The traditional
method assumes that respondents can accurately report how many people
they know in the hidden population and that all members are equally
likely to be known. These assumptions often fail due to stigma, recall
issues, or social barriers.
...
awareness. We apply these adjustments with sensitivity analysis across
multiple parameter values: δ ∈ \{0.8, 0.9, 1.0\} and τ ∈ \{0.7, 0.85,
1.0\}, where δ=1.0 and τ=1.0 represent the unadjusted baseline scenario.

\subsubsection{Comparative Rationale: Ego-Based vs.~Alter-Based
...
fundamentally different information for inference. \textbf{RDS uses
ego-based information}: respondents report their own characteristics and
network degree, which are combined with recruitment wave to adjust for
unequal inclusion probabilities. The underlying logic is that
individuals with larger networks are more likely to be recruited,
creating bias toward highly connected respondents. RDS estimators
explicitly correct for this by weighting observations according to
network degree and recruitment path.

\textbf{NSUM uses alter-based information}: respondents serve as
informants about the wider hidden population, reporting how many people
they know with specific traits. These reports are scaled up using
assumptions about network size and visibility to infer prevalence in the
broader population. NSUM does not depend on recruitment chains but
instead on the accuracy of respondents' knowledge about others in their
social circles.
...
\subsubsection{Bootstrap and Sensitivity
...
adjustment factor values. Full results are reported in Appendix B.
...
Table~\ref{tbl-sample-characteristics} and Figure~\ref{fig-rds-network}
(Section 3.3). In total, 97 respondents completed the survey, of whom 85
had non-zero network degree and were included in RDS analysis. The
analytical sample comprised 90 transnational migrants (93\%), with the
largest nationality cluster being Filipino (66\%). Female domestic
workers constituted 97\% of the sample, with over half aged 45 or older.
Self-employment status was reported by 45\% of respondents.

\subsection{Model-Assisted Estimates of the Continuous Risk
Index}\label{model-assisted-estimates-of-the-continuous-risk-index}
...
with the ILO's forced labour framework, weighted inductively and refined
through expert consultation. Rather than treating exploitation as a
dichotomy, the risk index conceptualises all domestic workers as facing
some degree of potential exploitation, albeit with significant variation
in intensity.
...
\autocite{gile15-network}. These methods adjust for the non-random
structure of RDS recruitment while permitting reliable estimation of
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-ma-risk-estimates-1.pdf}
...
\subsection{Binary Exploitation Indicators: Comparative RDS--NSUM
Estimates}\label{binary-exploitation-indicators-comparative-rdsnsum-estimates}

To complement the continuous measure, we operationalised exploitation as
binary outcomes. Respondents were classified as exploited if they met
threshold indicators consistent with ILO definitions. We present
prevalence estimates from three approaches: (1) Model-Assisted (MA-RDS)
Bayesian estimates that adjust for RDS sampling bias, (2) frequentist
RDS estimators (RDS-I and RDS-SS) for comparison, and (3) Network
Scale-Up Method (NSUM) estimates with multiple adjustment factor
scenarios.
...
\caption{\label{fig-method-comparison}Comparative Method Overview: Point
estimates and uncertainty intervals for prevalence of exploitation
indicators across different estimation methods. RDS methods show 95\%
confidence intervals (bootstrap); Bayesian methods show 95\% credible
intervals (MCMC); NSUM shows estimates under different adjustment
scenarios. Baseline population assumption: 980,000 domestic workers.
Source: Authors' own work.}

\end{figure}%

Figure~\ref{fig-method-comparison} provides a comprehensive comparison
of prevalence estimates across all three approaches. While absolute
values differ---reflecting the fundamental differences between ego-based
(RDS) and alter-based (NSUM) inference---the overall pattern is
remarkably stable. Excessive hours consistently shows the highest
prevalence across all methods, followed by threats/abuse and pay-related
issues. Model-Assisted estimates generally fall between RDS frequentist
estimates and NSUM baseline estimates, though NSUM estimates are highly
sensitive to adjustment factor assumptions.

\subsubsection{Model-Assisted Bayesian
Estimates}\label{model-assisted-bayesian-estimates}

\begin{table}

\caption{\label{tbl-ma-binary-indicators}Model-Assisted Bayesian
Estimates of Binary Exploitation Indicators: Population prevalence
estimates adjusted for RDS sampling bias using Bayesian inference.
Values represent estimated proportions with 95\% credible intervals.
Source: Authors' Own Work.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-ma-binary-forest-1.pdf}
...
Bayesian MA-RDS (N=980,000, sample seed selection). Source: Authors' Own
Work.}

\end{figure}%

\subsubsection{Frequentist RDS
Estimates}\label{frequentist-rds-estimates}

Table~\ref{tbl-rds-binary-indicators} compares RDS-I and RDS-SS
estimators for binary exploitation indicators. As expected, RDS-SS
produces slightly more conservative estimates than RDS-I due to its
finite population correction. Both estimators show similar relative
patterns across indicators, with Figure~\ref{fig-rds-forest-plot}
visualizing the comparison.

\begin{table}

\caption{\label{tbl-rds-binary-indicators}Frequentist RDS Estimates of
Binary Exploitation Indicators: Comparison of RDS-I and RDS-SS
estimators with neighborhood bootstrap confidence intervals (N=980,000).
Source: Authors' Own Work.}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-rds-forest-plot-1.pdf}
...
\subsubsection{Network Scale-Up Method (NSUM)
Estimates}\label{network-scale-up-method-nsum-estimates}

Using the Modified Basic Scale-Up (MBSU) estimator
\autocite{feeh16-generalizing} with Sequential Sampling weights and a UK
domestic worker population of 980,000, Table~\ref{tbl-nsum-main}
presents prevalence estimates under three adjustment scenarios: (1)
unadjusted baseline (δ=1.0, τ=1.0), (2) moderate adjustments (δ=0.9,
τ=0.85), and (3) conservative adjustments (δ=0.8, τ=0.7).

NSUM estimates show substantial sensitivity to adjustment factor
assumptions. Unadjusted estimates range from 0.83\% (document
withholding) to 2.28\% (excessive hours). Conservative adjustments
accounting for network degree differences and visibility barriers
produce estimates approximately 1.8 times higher---ranging from 1.48\%
to 4.08\%---while moderate adjustments yield intermediate values
approximately 1.3 times the baseline. Wide confidence intervals reflect
substantial uncertainty inherent in network-based estimation methods,
particularly for small hidden populations.
...
domestic workers. Adjustment factors: \(\delta\) (δ, degree ratio)
controls for network size differences; \(\tau\) (τ, visibility/true
...
\bottomrule
\multicolumn{6}{l}{\rule{0pt}{1em}\textit{Note:} MBSU = Modified Basic Scale-Up method; δ = degree ratio (d̄\_H,F/d̄\_F,F, ratio of hidden population network degree to frame population degree); τ = visibility/true positive rate (v̄\_H,F/d̄\_H,F, proportion of edges correctly reported, measures transmission and reporting accuracy); Bootstrap method: neighborhood resampling with 1000 iterations; Population base: 980,000 UK domestic workers.}\\
\end{tabular}}
\endgroup{}
...
the findings. In Appendix B, a comprehensive analysis comparing all
prevalence estimates across indicators, methods, and parameter values is
presented. There, remarkable consistency is demonstrated across
different analytical approaches. The pattern of estimates is remarkably
stable across methods, with excessive hours consistently showing the
highest prevalence, followed by threats/abuse and pay-related issues,
while document withholding shows more moderate prevalence levels. This
...
This study estimated prevalence of labour exploitation among UK domestic
workers using both Respondent-Driven Sampling (RDS) and Network Scale-Up
Method (NSUM). We introduced a continuous risk index alongside binary
exploitation indicators based on ILO forced labour criteria.
Model-Assisted RDS estimates suggest prevalence rates ranging from 34\%
to 84\% for various exploitation indicators among respondents. In
contrast, NSUM estimates---highly sensitive to adjustment
factors---range from 0.83\% to 4.08\% depending on assumptions about
network degree differences (δ) and visibility/reporting accuracy (τ).
The pattern across methods is remarkably consistent: excessive working
hours shows the highest prevalence, followed by threats/abuse and
pay-related issues, while document withholding shows more moderate
levels. Robustness checks across different population size assumptions
and estimation approaches confirm the stability of these relative
patterns.
...
\emph{Dual conceptualisation of exploitation}: We introduce two distinct
...
\subsubsection{Interpreting RDS--NSUM Divergence: The Role of
Visibility}\label{interpreting-rdsnsum-divergence-the-role-of-visibility}
...
estimates suggest that 34\% to 84\% of respondents personally
experienced indicators of exploitation. In contrast, the corresponding
NSUM estimates suggest that only 2.9\% to 5.1\% of the general domestic
worker population experiences these same harms. This stark
divergence---often approaching an order of magnitude---warrants a
careful methodological explanation.
...
In contrast, RDS estimators such as the Volz--Heckathorn and especially
the Successive Sampling (SS) estimator rely on direct self-reporting of
hidden group membership. That is, RDS respondents are asked whether they
themselves belong to the group or exhibit a trait of interest. This
egocentric measurement bypasses the visibility problem entirely. While
RDS relies on its own set of assumptions---such as the random
recruitment assumption, accurate self-reported degree, and a
...
Salganik---including multiplicative correction factors for degree ratio
(\(\delta\) δ, network size differences), visibility/true positive rate
(\(\tau\) τ, transmission and reporting accuracy), and frame ratio
(\(\phi_F\) φ\_F)---can mitigate these biases, but require additional
data or assumptions \autocite{feeh16-generalizing}.

In our setting, where RDS estimates suggest that over 50\% of
respondents have experienced some form of labour exploitation, but NSUM
estimates suggest only 3--5\% of the domestic worker population does so,
visibility bias is almost certainly the dominant factor explaining the
discrepancy. This is especially true in contexts involving exploitation,
abuse, or trafficking---where group membership is often concealed both
by victims and by observers. Respondents in the frame population may
simply be unaware that their contacts are experiencing exploitation,
leading to substantial underreporting.
...
\subsection{Policy and Theoretical
Implications}\label{policy-and-theoretical-implications}
...
across industries. The differences in NSUM and RDS estimates warrants
further investitation and suggests research into \emph{combining}
multiple estimates may be a fruitful direction in human rights research
and especially prevelenace of modern slavery research.

\section{Conclusion}\label{conclusion}

This article examines original quantitative data collected from domestic
workers based in the UK to measure and assess the nature and prevalence
of labour exploitation among this vulnerable population. Our
contribution lies in the presentation of two different statistical
estimators based upon RDS and NSUM techniques and the construction of
both binary and composite risk indices from indicators recognised by the
International Labour Organization. In so doing, we shed new light on the
nature and extent of labour exploitation risk among these vulnerable
workers, who enter the UK on restricted visas in the hope of decent
work. The results of our analysis raise important questions for both
theory and practice about the range and accuracy of the estimates that
it is possible to generate using these statistical techniques.
Meaningful and trustworthy estimates of labour exploitation among such
vulnerable groups are required if policymakers are to use prevalence
measures to make sensitive decisions about policy adjustments and the
allocation of scarce resources to ensure entrants to the UK labour
market experience decent working conditions. We believe our study opens
the way for further research on the efficacious use of statistical
estimators to measure the size of hidden populations of exploited
workers and the identification of appropriate means to quantify the
extent of labour exploitation reported.
...
\subsection{The Survey Questions}\label{the-survey-questions}
...
respondent knows with certain experiences, used for NSUM). The
fundamental challenge is to create a fair comparison between the
``egocentric'' RDS questions (about personal experience) and the broader
``how many do you know'' NSUM questions. This was accomplished by first
identifying thematic links between questions and then implementing a
specific coding scheme to transform them into a consistent format.
...
\tightlist
\item
  Document Withholding (Most Confident): Comparing personal experience
  (Q70) with knowing others who lack access to their documents (Q71).
\item
  Pay/Debt Issues (High Confidence): Linking personal experience with
  debt (Q39) and withheld pay (Q42) to knowing others with such problems
  (Q43).
\item
  Threats/Abuse (High Confidence): Aggregating personal experiences of
  being forced, threatened, intimidated, or verbally abused (Q45, Q47,
  Q48) and comparing them to knowing others who experienced threats or
  force (Q49).
\item
...
\item
  ...
  not (Q79). Coding and Transformation Process The conceptual mapping
  was then put into practice through a detailed coding process, which is
  explicitly documented in the project's R script.
\end{itemize}

The primary goal was to convert all variables into a binary (0/1)
format, representing ``No'' or ``Yes'' to experiencing or knowing
someone who experienced the issue.
...
\tightlist
\item
  RDS estimation uses ``egocentric'' questions, which focus on the
  respondent's personal experiences (e.g., ``Have you been forced to
  work?'').
\item
  NSUM estimation uses questions about the respondent's knowledge of
  others in their network (e.g., ``How many domestic workers do you know
  who have experienced\ldots{}''). The survey questions designed for
  these two purposes do not always match up directly, making a fair
  comparison between the estimates difficult.
\end{itemize}

The goal is to aggregate some of the specific ``egocentric'' questions
to make them comparable to the broader ``how many do you know'' NSUM
questions. To accomplish this, several sets of ``linking'' questions
have been identified with varying degrees of confidence in their
comparability:
...
\item
  ...
  \tightlist
  \item
    RDS Question: Q70 asks if the employer has withheld the respondent's
    travel and identity documents.
  \item
    NSUM Question: Q71 asks how many other domestic workers the
    respondent knows who do not have access to their own documents.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    RDS Questions: This involves combining Q39 (having to pay off debt
    to someone who helped find work) and Q42 (has your pay ever been
    withheld?).
  \item
    NSUM Question: Q43 asks how many others the respondent knows who
    have experienced problems with debt or pay.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    RDS Questions: This requires grouping Q45 (forced, deceived, or
    threatened into poor working conditions), Q47 (employer intimidation
    or threats), and Q48 (verbal abuse).
  \item
    NSUM Question: Q49 asks how many others the respondent knows who
    have experienced the use of threat or force.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    RDS Questions: Q61 (weekly rest does not last 24 hours) and Q62
    (working excessive overtime) are combined.
  \item
    NSUM Question: Q64 asks about knowing others who work excessive
    overtime, lack breaks, or lack annual leave. The comparison here is
    considered ``flakier'' because the RDS questions do not ask about
    annual leave, creating a partial mismatch.
  \end{itemize}
\item
  ...
  \tightlist
  \item
    RDS Question: Q78 asks if the respondent knows who might help them
    (coded for ``No'' answers).
  \item
    NSUM Question: Q79 asks how many other domestic workers the
    respondent knows who do NOT know where to go for help.
  \end{itemize}
\end{itemize}

\newpage

\section{Appendix B}\label{app-sens}
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-rds-sensitivity-popsize-1.pdf}
...
methods. Figure~\ref{fig-ma-sensitivity-by-indicator} and
Figure~\ref{fig-ma-sensitivity-comparison} show MA estimates are robust
to these paramter choices.
...
\includegraphics[width=1\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-ma-sensitivity-by-indicator-1.pdf}
...
\includegraphics[width=0.95\linewidth,height=\textheight,keepaspectratio]{IJOPM_paperDraft_202051012b_files/figure-pdf/fig-ma-sensitivity-comparison-1.pdf}
...
\subsubsection{Sensitivity to Weight
...
\subsubsection{Sensitivity to NSUM Method and Adjustment
...
adjustments (δ\(\delta=0.8\) τ\(\tau=0.7\)) produced estimates
approximately 1.8 times higher than unadjusted estimates. Moderate
adjustments (δ=0.9, τ=0.85) provided intermediate values approximately
1.3 times higher than baseline, representing our preferred specification
that acknowledges likely visibility and reporting barriers while
avoiding extreme assumptions.
...
\bottomrule
\multicolumn{6}{l}{\rule{0pt}{1em}\textit{Note:} MBSU = Modified Basic Scale-Up; GNSUM = Generalized Network Scale-Up Method; δ = degree ratio (network size differences); τ = visibility/true positive rate (transmission and reporting accuracy). Adjustment factors control for network degree differences and visibility/reporting biases in network-based estimation.}\\
\end{tabular}}
\endgroup{}
...
\subsubsection{Summary of Sensitivity
Analyses}\label{summary-of-sensitivity-analyses}

Across all 64 parameter combinations tested (4 bootstrap methods × 4
population sizes × 2 weight methods × 2 NSUM methods), prevalence
...
Respondent-Driven Sampling (RDS). RDS introduces specific structural
...
typically derived from known degree-based estimators such as RDS-I, or
RDS-II or Gile's Successive Sampling (SS) weights.
...
\begin{itemize}
\item
  \(\delta\) (δ): Degree ratio (d̄\_H,F/d̄\_F,F) --- ratio of average
  network degree of hidden population to frame population,
\item
  \(\tau\) (τ): Visibility / true positive rate (v̄\_H,F/d̄\_H,F) ---
  proportion of network edges correctly reported; measures transmission
  and reporting accuracy; captures visibility of hidden population to
  frame population,
\item
  \(\rho\) (ρ): Popularity bias (relative visibility of hidden
  population members).
\end{itemize}
...
\subsection{Step 4: Aggregating Bootstrap
