...
>>>>>>> dab298d3b2ea029411e85f66f6cb92cd9c8b2dcf
...
if (file.exists(nsum_file)) {
  ...
  if (exists("enhanced_results")) {
    ...
    nsum_indicators <- c(
      ...
      "threats_abuse" = "Threats/Abuse",
      "excessive_hours" = "Excessive Hours",
      "access_to_help" = "Limited Access to Help"
    )

    # Extract NSUM estimates from enhanced_results
    nsum_summary <- enhanced_results %>%
      dplyr::filter(indicator_name %in% names(nsum_indicators)) %>%
      dplyr::group_by(indicator_name) %>%
      dplyr::slice(1) %>%  # Take first scheme for each indicator
      dplyr::ungroup() %>%
      dplyr::mutate(
        label = nsum_indicators[indicator_name]
      ) %>%
      dplyr::select(
        indicator = indicator_name,
        label,
        nsum_est = nsum_prevalence,
        nsum_ci_lower,
        nsum_ci_upper
      )

    if (nrow(nsum_summary) > 0) {
      # Format for display (estimates are already prevalences)
      nsum_display <- nsum_summary %>%
        ...
        dplyr::arrange(desc(nsum_summary$nsum_est))

      # Create table
      nsum_display %>%
        kableExtra::kbl(
          col.names = c("Exploitation Indicator", "NSUM Estimate (95% CI)"),
        ...
        kableExtra::footnote(
          general = "Network Scale-Up Method prevalence estimates with bootstrap confidence intervals from enhanced NSUM analysis.",
          general_title = "Note:",
          footnote_as_chunk = TRUE
        )
    } else {
      cat("No NSUM estimates available.\n")
    }
  } else {
    cat("enhanced_results object not found in file.\n")
  }
} else {
  cat("enhanced_nsum_results.RData file not found.\n")
}
```
...
calculate_chain_stats <- function(data) {
  ...
  get_chain_depth <- function(node_id, chains, depth = 0, visited = c()) {
    if (node_id %in% visited) return(depth)  # Prevent cycles
    ...
    if (length(children) == 0) {
      ...
      child_depths <- sapply(children, function(child) {
  ...
  chain_lengths <- sapply(seeds, function(seed_id) {
  ...
  for (seed_id in seeds) {
    ...
    if (nrow(descendants) > 0) {
      for (desc_id in descendants$id) {
        all_depths <- c(all_depths, get_chain_depth(desc_id, chains, depth = 1))
      }
    }
  }

  return(list(
    longest = max(chain_lengths, na.rm = TRUE),
...
#| tbl-cap: "Comprehensive RDS Estimates: Prevalence estimates for all indicators across population sizes and RDS methods (RDS-I, RDS-II, RDS-SS) with neighborhood bootstrap confidence intervals."
#| echo: false
#| warning: false
...
if (file.exists(here("output", "frequentist_estimators_results.RData"))) {
  ...
  if (exists("frequentist_df")) {
    # Create comprehensive table with all methods and population sizes
    ...
    rds_comprehensive %>%
      kableExtra::kbl(
        booktabs = TRUE,
        align = c("l", rep("r", ncol(rds_comprehensive) - 1))
      ) %>%
      kableExtra::kable_styling(
      ...
      kableExtra::footnote(
        general = "RDS-I: Salganik-Heckathorn estimator. RDS-II: Volz-Heckathorn estimator. RDS-SS: Gile's Successive Sampling estimator. Population sizes: 50K, 100K, 980K, 1.74M domestic workers. All confidence intervals from neighborhood bootstrap (n=1000).",
        general_title = "Note:",
        footnote_as_chunk = TRUE
      )
  } else {
