...
get_bootstrap_confidence_intervals <- function(outcome_vars, confidence_level = 0.95, n_bootstrap = 500) {
  
  cat("=== Computing Bootstrap Confidence Intervals ===\n")
  cat("Method: Simple RDS bootstrap for", length(outcome_vars), "variables\n")
  ...
  for (outcome_var in outcome_vars) {
    ...
    tryCatch({
      ...
      # Simple bootstrap approach using RDS package
      bootstrap_estimates <- replicate(n_bootstrap, {
        
        # Resample with replacement
        n_sample <- nrow(rd.dd)
        bootstrap_indices <- sample(1:n_sample, size = n_sample, replace = TRUE)
        bootstrap_data <- rd.dd[bootstrap_indices, ]
        
        # Recompute RDS-SS estimate
        rds_ss <- RDS.SS.estimates(rds.data = bootstrap_data,
                                   outcome.variable = outcome_var,
                                   N = rds_config$main_population_size)
        
        return(rds_ss$estimate)
      })
      
      # Remove any NAs from bootstrap estimates
      ...
      if (length(bootstrap_estimates) > 0) {
        ...
        bootstrap_results[[outcome_var]] <- list(
          ci_lower = ci[1],
          ci_upper = ci[2],
          bootstrap_se = sd(bootstrap_estimates, na.rm = TRUE),
          n_successful = length(bootstrap_estimates)
        )
        
        cat("  CI: [", round(ci[1], 4), ",", round(ci[2], 4), "]\n")
      }
...
extract_preferred_results <- function(basic_results, preferred_method = "RDS_SS", 
                                    main_pop_size = 980000, include_bootstrap_ci = TRUE) {
  ...
  if (include_bootstrap_ci) {
    bootstrap_ci <- get_bootstrap_confidence_intervals(names(preferred_results))
    
    # Add CI information to preferred results
    for (outcome_var in names(preferred_results)) {
      if (outcome_var %in% names(bootstrap_ci)) {
        preferred_results[[outcome_var]]$ci_lower <- bootstrap_ci[[outcome_var]]$ci_lower
        preferred_results[[outcome_var]]$ci_upper <- bootstrap_ci[[outcome_var]]$ci_upper
        preferred_results[[outcome_var]]$bootstrap_se <- bootstrap_ci[[outcome_var]]$bootstrap_se
      } else {
        preferred_results[[outcome_var]]$ci_lower <- NA
        preferred_results[[outcome_var]]$ci_upper <- NA
        preferred_results[[outcome_var]]$bootstrap_se <- NA
      }
    }
  }
  ...
  main_table <- data.frame(
    ...
    mutate(
      ...
      indicator_base = str_remove(indicator, "_rds$"),
      indicator_clean = get_comparable_indicators()$labels[indicator_base],
      
      # Add confidence intervals if available
      ci_lower = if(include_bootstrap_ci) sapply(preferred_results, function(x) x$ci_lower) else NA,
      ci_upper = if(include_bootstrap_ci) sapply(preferred_results, function(x) x$ci_upper) else NA,
      
      # Create formatted estimate with CI
      estimate_with_ci = mapply(function(est, ci_l, ci_u) {
        if (!is.na(ci_l) && !is.na(ci_u)) {
          format_estimate_with_ci(est, ci_l, ci_u, as_percentage = TRUE)
        } else {
          sprintf("%.1f%%", est * 100)
        }
      }, estimate, ci_lower, ci_upper, SIMPLIFY = TRUE)
    ) %>%
    arrange(desc(estimate))
  
  cat("Preferred results extracted for", nrow(main_table), "indicators\n")
  if (include_bootstrap_ci) {
    cat("Bootstrap confidence intervals included\n")
  }
  cat("\n")
...
run_sensitivity_analysis <- function(basic_results, focus_vars = NULL) {
  ...
  for (outcome_var in focus_vars) {
    ...
    if (length(estimates_by_pop) > 0) {
      # Convert names to numeric, handling any potential issues
      pop_sizes_numeric <- suppressWarnings(as.numeric(names(estimates_by_pop)))
      pop_sizes_numeric <- pop_sizes_numeric[!is.na(pop_sizes_numeric)]
      
      pop_sensitivity[[outcome_var]] <- list(
        population_sizes = pop_sizes_numeric,
