...
vis <- impute.visibility(
  rds,
  type.impute = "median",     # or "mean" or "mode"
  recruit.time = "wave",       # or your interview time variable
  include.tree = TRUE,         # recommended; see below
  burnin = 5000, interval = 10,
  parallel = 1,                # keep simple while debugging
  verbose = TRUE
)
...
V <- impute.visibility(
  rds,
  type.impute = "median",  # ignored for the matrix we request
  return.posterior.sample.visibilities = TRUE,
  recruit.time = "wave",
  include.tree = TRUE,
  burnin = 5000, interval = 10,
...
set.seed(1)
vis <- apply(V, 2, function(x) sample(x, 1))


V <- impute.visibility(
  rds,
  type.impute  = "median",        # avoids the fragile "distribution" post-step
  include.tree = TRUE,
  recruit.time = "wave",          # or your time variable
  burnin = 5000, interval = 10,
  parallel = 1,                   # keep simple while debugging
  verbose = TRUE,
  return.posterior.sample.visibilities = TRUE
...
set.seed(1)
vis_draw <- apply(V, 2, function(x) sample(x, 1))  # one posterior draw per person





# A. safer, explicit imputation (no brittle branch)
V <- impute.visibility(
  rds,
  type.impute  = "median",            # safe mode (we’ll ignore this for V)
  include.tree = TRUE,                 # stabilizes visibilities
  recruit.time = "wave",               # or your time variable; else omit
  median.prior.size = 980000,          # <-- removes prior warning
  burnin = 5000, interval = 10,
  parallel = 1,                        # keep simple while debugging
  verbose = TRUE,
  return.posterior.sample.visibilities = TRUE
...
# B. now emulate "distribution": one posterior draw per respondent
set.seed(123)
vis_draw <- apply(V, 2, function(x) sample(x, 1))
...
# (A) Visibility imputation to add stochasticity
vis <- impute.visibility(rds, type.impute = "distribution", burnin = 5000, interval = 10)

# (B) Longer, tuned run
fit <- posteriorsize(
  rd.dd,
  burnin = 60000, interval = 25, samplesize = 40000,
  mu_proposal = 0.3,  nu_proposal = 0.4,
  beta_0_proposal = 0.6, beta_t_proposal = 0.025, beta_u_proposal = 0.025,
  memmu_proposal = 0.3, memscale_proposal = 0.4,
  visibilitydistribution = "nbinom",   # if supported and overdispersion is high
  K = 60,                               # if many degrees at the cap 35
  seed = 11
)


# (C) Diagnostics you should always look at
png("sspse_traces.png", 1600, 900, res = 150)
par(mfrow = c(3,3), mar = c(3,3,2,1)); plot(fit, mcmc = TRUE)
dev.off()

ess <- coda::effectiveSize(fit$sample)
print(ess)  # aim ESS >= 200–400 for mu/sigma/betas

# Posterior-predictive degree check
png("sspse_ppc_degree.png", 1200, 900, res = 150)
plot(pospreddeg(fit))
dev.off()
