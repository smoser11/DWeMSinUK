...
void lcmp2 (int *pop,
  ...
  while (isamp < isamplesize) {
    ...
    if (step == -iburnin || step==(10*(step/10))) {
     MHlcmp2(Nk,K,mu,kappa,sigma,df,muproposal,sigmaproposal,
           &Ni, &Np, psample,
    ...
    pis=0.;
    lzcmp = zcmp2(exp(mui), sigmai, errval, Ki, give_log1);
    if(lzcmp < -100000.0){continue;}
    pi[Np]=cmp2(Np+1,mui,sigmai,lzcmp,give_log0);
    for (i=Np+1; i<Ki; i++){
...
//    Rprintf("mustar %f sigmastar %f lzcmp %f\n", mustar, sigmastar, lzcmp);
    pstars=0.;
    lzcmp = zcmp2(exp(mustar), sigmastar, errval, Ki, give_log1);
    pstar[Np]=cmp2(Np+1,mustar,sigmastar,lzcmp,give_log0);
    for (i=Np+1; i<Ki; i++){
