...
double cmp2(int x, double llambda, double nu, double lzcmp, int give_log)
  {
     double dev;
...
double zcmp2(double lambda, double nu, double err, int K, int give_log)
  {
     double mj=0.0, z, aj;
     int j;
     z = 1.0 + lambda;
...
void dcmp2 (int *x, double *lambda, double *nu, int *n, double *err, int *give_log, double *val) {
  ...
  lzcmp = zcmp2(*lambda, *nu, *err, 2*xmax, give_log1);
//Rprintf("lzcmp %f \n", lzcmp);
  for (i = 0; i < *n; i++){
    val[i] = cmp2(x[i], log(*lambda), *nu, lzcmp, *give_log);
  }
}

void rcmp2 (int *x, double *lambda, double *nu, int *n, int *K, double *err) {
  ...
  lzcmp = zcmp2(*lambda, *nu, *err, 2*Ki, give_log1);
  for (i = 0; i < Ki; i++){
    pi[i] = cmp2(i, llambda, *nu, lzcmp, give_log0);
  }
  for (i=1; i< Ki; i++){
    pi[i]=pi[i-1]+pi[i];
  }
