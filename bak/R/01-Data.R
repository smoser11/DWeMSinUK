...
## Create comparable RDS/NSUM indicators
create_comparable_indicators <- function(data) {
	library(car)
	
	# 1. Document withholding (most confident)
	...
	# 2. Pay/debt issues (high confidence)  
	...
	# 3. Threats/abuse/force (high confidence)
	...
	# 4. Excessive hours (lower confidence - missing annual leave in RDS)
	...
	# Q64: Know others with labour rights issues? (already numeric count)
	...
	# 5. Access to help (lowest confidence - reverse coding needed)
	# Q78: Do you know who might help if not properly paid/treated?
	data$zQ78 <- recode(data$q78, "1 = 1; 0 = 0")  # No = 1 (vulnerable), Yes = 0
